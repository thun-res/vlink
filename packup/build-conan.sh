...
#   - Produces:
#       1) portable archive  : build-conan/packup/<linux|darwin>/vlink-desktop-<ver>-<os>-<arch>.tgz
#       2) QtIFW installer   : build-conan/packup/<linux|darwin>/vlink-desktop-<ver>-<os>-<arch>(.app)
...
command -v conan &> /dev/null || pip install conan==2.29.1 --user
...
if [ "$PLATFORM_OS" = "Darwin" ];then
    ...
    if [ ! -z $QT_DIR ];then
        ...
        if [[ ${QT_VERSION} = 5.* ]] || [[ $QT_VERSION = 6.* ]];then
            for _fw in QtCore QtDBus QtGui QtWidgets QtOpenGL QtOpenGLWidgets QtNetwork QtSql QtSvg;do
...
EOL
    ...
    for f in run_viewer.sh run_player.sh run_analyzer.sh run_cmd.sh;do
        [ -f $WORK_DIR/linux/$f ] && cmake -E copy $WORK_DIR/linux/$f $PACKUP_DIR/bin/
    done
    [ -f $WORK_DIR/linux/install.sh ] && cmake -E copy $WORK_DIR/linux/install.sh $PACKUP_DIR/
    [ -f $WORK_DIR/linux/uninstall.sh ] && cmake -E copy $WORK_DIR/linux/uninstall.sh $PACKUP_DIR/
    [ -f $WORK_DIR/linux/setup_runtime.sh ] && cmake -E copy $WORK_DIR/linux/setup_runtime.sh $PACKUP_DIR/
    cmake -E copy $WORK_DIR/linux/qt.conf $PACKUP_DIR/bin/
    chmod +x $PACKUP_DIR/bin/*.sh $PACKUP_DIR/*.sh 2>/dev/null
    for f in $(find $WORK_DIR/linux -maxdepth 1 \( -name '*.desktop' -o -name '*.png' -o -name '*.svg' -o -name '*.xml' \));do
        cmake -E copy $f $PACKUP_DIR/desktop/
    done
    if [ -d "$WORK_DIR/linux/$PLATFORM_ARCH" ]; then
...
ARCHIVE_NAME="vlink-desktop-${VERSION}-${PLATFORM_TAG}.tgz"
...
if [ "$PLATFORM_OS" = "Darwin" ]; then
    OUTPUT_NAME="vlink-desktop-${VERSION}-${PLATFORM_TAG}"
else
    OUTPUT_NAME="vlink-desktop-${VERSION}-${PLATFORM_TAG}.run"
fi
