...
::   - Produces:
::       1) portable archive  : build-conan\packup\win32\vlink-desktop-<ver>-windows-<arch>.zip
::       2) QtIFW installer   : build-conan\packup\win32\vlink-desktop-<ver>-windows-<arch>.exe
::
...
where conan >nul 2>nul || pip install conan==2.29.1 --user
...
findstr /C:"CMAKE_SIZEOF_VOID_P:INTERNAL=4" "%BUILD_DIR%\CMakeCache.txt" >nul 2>nul
if %errorlevel% equ 0 (
    echo Error: build-conan.bat packages x64 artifacts, but CMake configured a 32-bit build.
    echo Use an x64 generator or run from an x64 Visual Studio Developer Command Prompt.
    exit /b 2
)
...
set "TERMINAL_URL=https://github.com/microsoft/terminal/releases/download/v1.24.11321.0/Microsoft.WindowsTerminal_1.24.11321.0_x64.zip"
set "TERMINAL_ZIP=%BUILD_DIR%/downloads/terminal.zip"
set "TERMINAL_TMP=%BUILD_DIR%/terminal-extract"
set "TERMINAL_DST=%PACKUP_DIR%/terminal"

echo Downloading Windows Terminal: %TERMINAL_URL%
cmake -E make_directory "%BUILD_DIR%/downloads"
powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri $env:TERMINAL_URL -OutFile $env:TERMINAL_ZIP"
if %errorlevel% neq 0 (
    echo Error: failed to download Windows Terminal.
    exit /b 2
)
if exist "%TERMINAL_TMP%" rmdir /s /q "%TERMINAL_TMP%"
if exist "%TERMINAL_DST%" rmdir /s /q "%TERMINAL_DST%"
powershell -NoProfile -ExecutionPolicy Bypass -Command "Expand-Archive -Path $env:TERMINAL_ZIP -DestinationPath $env:TERMINAL_TMP -Force"
if %errorlevel% neq 0 (
    echo Error: failed to extract Windows Terminal.
    exit /b 2
)
for /d %%d in ("%TERMINAL_TMP%\*") do if not exist "%TERMINAL_DST%" move "%%~fd" "%TERMINAL_DST%" >nul
if not exist "%TERMINAL_DST%\wt.exe" (
    echo Error: Windows Terminal wt.exe not found after extraction.
    exit /b 2
)
rmdir /s /q "%TERMINAL_TMP%" >nul 2>&1
...
set "ARCHIVE_NAME=vlink-desktop-%VERSION%-windows-x64.zip"
...
cmake -E make_directory "%ARCHIVE_DIR%"
if exist "%ARCHIVE_PATH%" del /f /q "%ARCHIVE_PATH%"
cmake -E chdir "%ARCHIVE_DIR%" cmake -E tar cf "%ARCHIVE_NAME%" --format=zip vlink
if %errorlevel% neq 0 (
...
set "OUTPUT_NAME=vlink-desktop-%VERSION%-windows-x64"
