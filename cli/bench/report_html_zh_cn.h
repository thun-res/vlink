...
inline std::string render_i18n_script_json_zh_cn() {
  return reinterpret_cast<const char*>(
    ...
    "profile_note": "按本次总分从高到低排列。每张卡对应一个被测 URL/传输配置；先看总分、延迟、吞吐、CPU 和置信度，点开后看 URL、运行方式、最好/最弱延迟 case 和典型指标。",
    "profile_fact_url_prefix": "协议/传输",
    "profile_fact_deployment": "运行方式",
    "profile_num_best_p95": "最好 P95 延迟",
    "profile_num_weakest_latency": "最弱延迟用例",
    ...
    "metric_p95_meaning": "延迟百分位。热力图展示 P50（典型）/ P90（偏慢的常见值）/ P99（尾部）三档；详细表还会展示 P99.9、P99.99，用来暴露极少数慢消息。数值越小越好。",
    ...
    "suite_score_factors_latency": "以 P95/P99 延迟质量为主，少量参考 P99.9/P99.99、最大延迟、抖动、吞吐和健康状态。每个 case 只贡献一个 case 权重。",
    "suite_score_gates_latency": "延迟测试主要看百分位：热力图展示 P50（典型）/ P90（偏慢的常见值）/ P99（尾部）；P99.9 / P99.99 仍作为尾部稳定性信号展示和轻量扣分，但不会主导得分。数值越低越好。",
    "suite_score_interp_latency": "适合判断消息响应是否够快、慢消息是否可控。P95/P99 主导得分，P99.9/P99.99 用来解释罕见卡顿。",
    ...
    "profile_formula_latency_desc": "— 订阅端端到端延迟。每个 case 内部按 P95×0.70 + P99×0.25 + P99.9×0.04 + P99.99×0.01 加权，再过 payload 感知曲线（excellent_us = clamp(80 + payload_KiB·0.40, 80, 1500)，poor = excellent×8，中间对数插值）；跨 case 使用等权几何平均，每个 payload size 都只贡献一个 case 权重。",
    ...
    "profile_formula_send_block_desc": "— publisher 调用 publish() 进入到返回的墙上钟阻塞时长。延迟和 backpressure case 的 P99 阻塞时间会过 payload 感知曲线（excellent = clamp(5 + payload_KiB·0.05, 5, 200) µs，poor = excellent×30）；跨 case 使用等权几何平均。**惩罚那些占着发布线程不放的 API**；异步入队即返回的 API 会自然拿高分。",
