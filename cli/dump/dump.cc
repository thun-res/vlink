...
int main(int argc, char* argv[]) {
  ...
  if (!condition.empty()) {
    auto raw_field_specs = vlink::Helpers::get_split_string(condition, ',');
    ...
    for (auto& spec : raw_field_specs) {
      ...
      ctx.field_paths.emplace_back(vlink::Helpers::get_split_string(spec, '.'));
    }
  }
  ...
  if (!expr_raw.empty()) {
    ctx.expr_strings = vlink::Helpers::get_split_string(expr_raw, ';');
