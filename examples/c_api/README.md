...
- **Schema 元信息**：创建函数的 `vlink_schema_info_t*` 参数可传 `NULL`；若提供，则 `ser` 与 `schema` 必须成对有效。它与 C++ 侧的 `Node::set_ser_type()` 对应，并传播至 Discovery、ProxyServer、BagWriter、Viewer；错填会导致对端无法正确识别载荷。
