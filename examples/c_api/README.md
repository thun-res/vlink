...
- **Schema 元信息**：所有创建函数都要求传入 `vlink_schema_info_t { const char* ser; vlink_schema_t schema; }`，与 C++ 侧的 `Node::set_ser_type()` 一一对应，并传播至 Discovery、ProxyServer、BagWriter、Viewer。这是真实的 wire 元数据，错填会导致跨语言/跨进程对端无法识别。
...
跨语言互操作（C / C++ / Python）仅需 URL、`ser_type`、`schema_type` 三元组一致即可直接对接。
