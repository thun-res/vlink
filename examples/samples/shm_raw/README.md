...
单进程内用 `shm://`（Iceoryx 共享内存后端）一次性演示 VLink 全部六种通信原语，且全部走 `Security*` 加密变体。安全层先生成密文缓冲区，SHM 后端再复制进共享内存池，因此本示例不是端到端零拷贝。适用场景：同主机沙箱进程间通信，即使数据不出本机也要求机密性。
...
| 跨主机或需服务发现 | 使用地址兼容的 `dds://`、`zenoh://` URL；通信原语不变 |
