...
在受支持的通信原语上调用 `set_record_path("/tmp/x.vdb")`，流经该节点可序列化录制路径的消息将自动写入 bag 文件；或设置环境变量 `VLINK_BAG_PATH`，为进程建立全局 writer。节点级 `set_record_path()` 不支持 `intra://` 与 DDS CDR；全局 writer 仍可记录走 Bytes 序列化路径的普通 intra 消息，但不记录 `IntraData` 直通消息或 DDS CDR。
...
| API | 签名 | 用途 |
|-----|------|------|
| `set_record_path` | `void set_record_path(const std::string& path)` | 六个原语均提供该 API；空串关闭。`intra://` 与 DDS CDR 调用会报 fatal，不支持录制 |
| `BagWriter::global_get` | `static BagWriter* global_get()` | 取全局 writer；非空表示 `VLINK_BAG_PATH` 指向受支持格式且 writer 创建成功 |
| 环境变量 `VLINK_BAG_PATH` | path | 启动时自动建立全局 BagWriter；不记录 `IntraData` 直通消息或 DDS CDR |
