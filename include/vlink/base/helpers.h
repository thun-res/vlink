...
/**
 * @brief Splits a filter string by space or comma; each token is trimmed and empty tokens are skipped.
 *
 * @param str  Source filter string.
 * @return Vector of trimmed, non-empty filter tokens.
 */
[[nodiscard]] VLINK_EXPORT std::vector<std::string> get_split_filter(const std::string& str) noexcept;
