...
inline MpmcQueue<T>::MpmcQueue(size_t capacity) : MpmcQueueBase(capacity) {
  ...
  if VUNLIKELY (reinterpret_cast<size_t>(raw) % alignof(Chunk) != 0U) {
    allocator.deallocate(raw, capacity_ + 1);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    throw_mpmc_alignment_failure();            // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
inline void MpmcQueue<T>::emplace(ArgsT&&... args) noexcept {
  ...
  const auto head = head_.fetch_add(1U, std::memory_order_seq_cst);
...
inline void MpmcQueue<T>::pop(T& v) noexcept {
  auto const tail = tail_.fetch_add(1U, std::memory_order_seq_cst);
...
inline bool MpmcQueue<T>::try_pop(T& v) noexcept {
  ...
  for (;;) {
    ...
    if VLIKELY (turn(tail) * 2U + 1U == chunk.turn.load(kMemoryOrderAcquire)) {
      if (tail_.compare_exchange_strong(tail, tail + 1U, std::memory_order_seq_cst, std::memory_order_seq_cst)) {
        v = chunk.move();
        chunk.destroy();
        chunk.turn.store((turn(tail) * 2U) + 2U, kMemoryOrderRelease);
