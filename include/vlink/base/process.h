 ...
 * | Process metadata      | @c get_process_id, @c get_state, @c get_error, @c get_exit_code          |
...
class VLINK_EXPORT Process {
 public:
  ...
  /**
   * @brief Blocks until the child leaves @c kStartingState or the timeout elapses.
   *
   * @details A child that already exited still counts as started.
   *
   * @param msecs  Maximum wait in milliseconds.  Default: @c kDefaultWaitTimeoutMs.
   * @return @c true if the child started (even if since exited); @c false on launch
   *         failure, timeout, or if never started.
   */
