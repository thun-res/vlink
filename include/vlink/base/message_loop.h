...
class VLINK_EXPORT MessageLoop {
 public:
  ...
  /**
   * @brief Schedules a @c void-returning callable and returns a chainable @c Schedule::Status.
   *
   * @details
   * @c Schedule::Config carries the delay, priority and timeout fields.  The returned status
   * supports @c on_catch / @c on_schedule_timeout / @c on_execution_timeout chaining.  Posting is
   * deferred until the handle is committed (when the temporary handle is destroyed at the end of
   * the fluent expression), so every continuation is registered before the task runs.  A caller
   * that stores the handle instead of consuming it inline must call @c Schedule::Status::dispatch()
  ...
  /**
   * @brief Schedules a @c bool-returning callable and returns a chainable @c Schedule::RetStatus.
   *
   * @details
   * @c on_then fires when the callable returns @c true; @c on_else fires when it returns @c false.
   * Posting is deferred until the handle is committed (destroyed at the end of the fluent
   * expression), guaranteeing every continuation is registered first; a stored handle must call
   * @c Schedule::Status::dispatch() to post it.
   *
...
Schedule::Status MessageLoop::exec_task(const Schedule::Config& config, CallbackT&& callback) {
  ...
  status.launcher_ = make_launcher(config, std::move(wrapper_callback), status.impl_);
