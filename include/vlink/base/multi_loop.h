 ...
 *
 * @par Dispatch behaviour (delegated to the underlying @c ThreadPool)
 *
 * Forwarded tasks are submitted to the pool's single shared queue.  Whichever worker is
 * idle dequeues the next task, woken by the pool's condition variable.  There is no
 * round-robin, least-loaded, or hash-based worker selection: cross-worker ordering and
 * thread affinity are not guaranteed.
 *
 * @par Worker diagram
 *
 * @verbatim
