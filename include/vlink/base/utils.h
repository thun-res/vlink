...
/**
 * @brief Returns whether an interface name is treated as virtual/tunnel-only by address discovery.
 *
 * @details Used by @c get_all_ipv4_address(true) and @c get_all_ipv6_address(true) to skip
 * container, VPN, bridge and tunnel adapters when choosing transport discovery addresses.
 *
 * @param name Interface name; @c nullptr is treated as ignored.
 * @return @c true when the name should be filtered out.
 */
 ...
 *
 * @details
 * When @c 127.0.0.1 is present it is placed first in the result; the remaining
 * non-loopback unicast addresses are then appended until @c max_count is reached.
 ...
 * @c /proc/stat on Linux/Android or @c GetSystemTimes on Windows.  Because the previous
 * sample is zero-initialised, the first call after process start reports the average usage
 * since boot rather than the usage over an interval.
 *
 * @return Percentage in @c [0.0, @c 100.0], or @c -1.0 on error / unsupported platform.
 */
