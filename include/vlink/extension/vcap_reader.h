...
/**
 * @file vcap_reader.h
 * @brief Indexed playback of MCAP-format bag files (@c .vcap / @c .vcapx).
 *
 * @details
 * @c VCAPReader concretises @c BagReader for the MCAP container used by Foxglove Studio and
 * other MCAP-compatible visualisers.  Splits captured into @c .vcapx manifests are followed
 * transparently; @c reindex() and @c fix() resolve to @c false because MCAP itself owns the
 * footer index and does not support in-place repair through this implementation.
 *
 * @par VCAP file format
 * @code
 *  +---------------------------------------------------------------+
 *  | MCAP magic + header                                           |
 *  +---------------------------------------------------------------+
...
/**
 * @class VCAPReader
 * @brief Concrete MCAP-format @c BagReader implementation with index-driven seek.
 *
 * @details
 * Prefer @c BagReader::create() for format-agnostic instantiation; instantiate this class
 * directly only when an MCAP-specific feature is required.
 */
class VLINK_EXPORT VCAPReader final : public BagReader {
 public:
  /**
   * @brief Opens an MCAP file for playback.
   *
   * @param path        Filesystem path of the @c .vcap or @c .vcapx file.
   * @param read_only   @c true blocks any in-place modification (default).
   * @param try_to_fix  @c true enables a fallback summary scan when the indexed summary is unreadable.
   */
  explicit VCAPReader(const std::string& path, bool read_only = true, bool try_to_fix = false);

  /**
   * @brief Stops playback and releases the MCAP file handle.
   */
  ~VCAPReader() override;
  ...
  /**
   * @brief Registers a callback fired once the file is open, parsed, and ready to play.
   *
  ...
  /**
   * @brief Unsupported MCAP reindex operation.
   *
   * @return Future that always resolves to @c false.
   */
  std::future<bool> reindex() override;

  /**
   * @brief Unsupported MCAP repair operation.
   *
  ...
  /**
   * @brief Updates the @c .vcapx manifest tag.  Single @c .vcap files are left untouched.
   *
  ...
  /**
   * @brief Enumerates every schema embedded in the MCAP file.
   *
  ...
  /**
   * @brief Reports whether the bag is split across multiple @c .vcap files.
   *
   * @return @c true when reading a @c .vcapx manifest with multiple parts.
   */
 ...
 private:
  bool prepare_cursor_view(int file_index);
