 ...
 * @endverbatim
 *
 * The process reporter is owned by the VLink runtime and is not exposed as a public
 * singleton.  @c add() and @c remove() are invoked automatically by
 * @c NodeImpl::init_ext() / @c NodeImpl::deinit_ext(); normal user code should rarely
 ...
 * vlink::DiscoveryReporter reporter;
 * reporter.async_run();
 * @endcode
 *
 ...
 * separated addresses) sends one report per listed address,
 * @c VLINK_DISCOVER_NATIVE=1 limits it to loopback and @c VLINK_DISCOVER_DOMAIN shifts the
 * discovery port to isolate the channel.
 */
...
class VLINK_EXPORT DiscoveryReporter : public MessageLoop {
 public:
   ...
   * The runtime-owned process reporter does this automatically.
   */
  DiscoveryReporter();
   ...
   *
   * @details
   * Invoked automatically by @c NodeImpl::init_ext().
   *
   ...
   *
   * @details
   * Invoked automatically by @c NodeImpl::deinit_ext().
   *
