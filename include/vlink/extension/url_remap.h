...
class VLINK_EXPORT UrlRemap {
 public:
   ...
   * @return Reference to the rewritten URL on a hit, or to @p url itself on a
   *         miss. The referent's lifetime depends on the path taken (the input
   *         @p url, the internal remap table, or the cache); copy it if you need
   *         to retain it.
   */
