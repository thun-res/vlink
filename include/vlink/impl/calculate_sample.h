 ...
 *
 * @par Algorithm
 * - The first call for a GUID seeds @c first with the observed sequence number
 *   and @c expected with @c seq + 1, and records zero loss.
 * - Subsequent in-order frames increment @c expected by one.
 * - When @c seq is ahead of @c expected by at most @c UINT32_MAX, the gap is
 *   added to @c lost and @c expected jumps to @c seq + 1.
 * - A gap strictly greater than @c UINT32_MAX is treated as a sender restart
 *   and the state is re-seeded with no extra loss recorded.
 *
