 ...
 * point, while the generic @c get_value() accessors return the raw @c int16_t
 * storage.  Points whose XYZ fall outside @c (-extent, +extent) cannot be represented and are
...
struct VLINK_EXPORT_AND_ALIGNED(8) PointCloud final {
   ...
   * @return @c false on protocol validation failure (including non-float / double XYZ when @p extent is non-zero).
   */
