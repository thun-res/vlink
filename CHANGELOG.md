...
- **Extension**：修复 Bag 录放与 Schema、空 Protobuf、URL 重映射、插件启动及 SQLite 关闭构建，减少 Schema 拷贝。
...
- **Python 稳定性**：修复浅视图与空数据深拷贝的生命周期问题，以及满队列投递、日志回调并发替换、Process 重启、Timer 回调释放的死锁风险。
