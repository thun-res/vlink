...
void VDBReader::open(const std::string& path) {
  ...
  try {
    ...
    if (suffix == ".vdbx") {
      try {
        ...
        for (const auto& file_info : files_json) {
          ...
          if VUNLIKELY (db_size_ec) {
            CLOG_W("VDBReader: file_size failed for [%s]: %s.", file_db_str.c_str(), db_size_ec.message().c_str());
            file_size = 0;
          }
      ...
      if VUNLIKELY (!to_open(wrapper_file)) {
        CLOG_F("VDBReader: Failed to prepare database [%s].", path.c_str());
        return;
      }
      ...
      if VUNLIKELY (db_size_ec) {
        CLOG_W("VDBReader: file_size failed for [%s]: %s.", path.c_str(), db_size_ec.message().c_str());
        file_size = 0;
      }
...
bool VDBReader::prepare_cursor_stmt(int file_index) {
  ...
  if VUNLIKELY (!wrapper_file.db) {
    VLOG_W("VDBReader: Cursor target db is empty.");
    return false;
  }
  ...
  if VUNLIKELY (ret != SQLITE_OK) {
    CLOG_W("VDBReader: Failed to prepare cursor stmt: %s.", ::sqlite3_errmsg(wrapper_file.db));
    return false;
  }
...
bool VDBReader::do_open_cursor(const Config& config) {
  ...
  if VUNLIKELY (impl_->file_list.empty()) {
    VLOG_W("VDBReader: Cursor cannot find any data.");
    return false;
  }
