...
[[maybe_unused]] static std::string resolve_program_path(const std::string& program,
  ...
  return program;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
}
...
[[maybe_unused]] static ssize_t write_no_sigpipe(int fd, const void* data, size_t size) {
  ...
  if VUNLIKELY (mask_error != 0) {
    errno = mask_error;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    return -1;           // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
#else
    const struct timespec zero_timeout{0, 0};                             // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    while (::sigtimedwait(&sigpipe_set, nullptr, &zero_timeout) == -1 &&  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
           errno == EINTR) {                                              // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
#endif
  }
  ...
  if VUNLIKELY (restore_error != 0) {
    errno = restore_error;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    return -1;              // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
struct Process::Impl final {  // NOLINT(clang-analyzer-optin.performance.Padding)
  ...
  DWORD process_id{0};
#else
  std::atomic<int64_t> process_id{-1};
...
#else
    // LCOV_EXCL_START GCOVR_EXCL_START
    CLOG_W("Process: Still running (PID=%d).", static_cast<int>(impl_->process_id.load(std::memory_order_acquire)));
#endif
...
#else
      // LCOV_EXCL_START GCOVR_EXCL_START
      CLOG_E("Process: Force to kill (PID=%d).", static_cast<int>(impl_->process_id.load(std::memory_order_acquire)));
#endif
...
#else
  return impl_->process_id.load(std::memory_order_acquire);
#endif
}

void Process::set_max_buffer_size(size_t size) {
  const size_t max_size = size > 0 ? size : kDefaultMaxBufferSize;
...
bool Process::wait_for_started(int msecs) {
  ...
  if VUNLIKELY (current_state == kNotRunningState) {
    return impl_->exit_processed.load(std::memory_order_acquire) &&
           impl_->error.load(std::memory_order_acquire) != kStartError;
  }
  ...
  return impl_->error.load(std::memory_order_acquire) != kStartError;
}
...
bool Process::wait_for_finished(int msecs) {
  ...
  if (pid > 0 && !impl_->exit_processed.load(std::memory_order_acquire)) {
    int status = 0;
    pid_t result = 0;
    ...
    if (result == pid) {
      ...
      // LCOV_EXCL_START GCOVR_EXCL_START
      std::unique_lock relock(impl_->state_mtx);
      return impl_->state_cv.wait_for(relock, std::chrono::milliseconds(kMonitorLoopDelayMs), [this]() {
        return impl_->state.load(std::memory_order_acquire) == kNotRunningState;
      });
    }
    // LCOV_EXCL_STOP GCOVR_EXCL_STOP
  }
...
void Process::terminate() {
  ...
  const int64_t pid = impl_->process_id.load(std::memory_order_acquire);
  ...
  if VLIKELY (pid > 0) {
    ...
    for (int i = 0; i < 20; ++i) {
      ...
      if (result == static_cast<pid_t>(pid)) {
        ...
        if (WIFEXITED(status)) {
          exit_code = WEXITSTATUS(status);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
          exit_status = kNormalExitStatus;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        } else if (WIFSIGNALED(status)) {
          int sig = WTERMSIG(status);
          exit_code = 128 + sig;
          exit_status = kCrashExitStatus;
...
bool Process::start_detached(const std::string& program, const std::vector<std::string>& arguments) {
  ...
  if VUNLIKELY (pid < 0) {
    ::close(exec_status_pipe[0]);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    ::close(exec_status_pipe[1]);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  ...
  if VUNLIKELY (pid == 0) {
    ...
    pid_t pid2 = fork();  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

    if VUNLIKELY (pid2 < 0) {
      int error_code = errno;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

      written = ::write(exec_status_pipe[1], &error_code, sizeof(error_code));  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

      (void)written;

      ::close(exec_status_pipe[1]);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

      _exit(kExecFailedExitCode);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }

    if VLIKELY (pid2 > 0) {
      ::close(exec_status_pipe[1]);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      _exit(0);                      // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
...
void Process::set_state(State state) {
  ...
  {
    std::lock_guard lock(impl_->state_mtx);
    old_state = impl_->state.exchange(state, std::memory_order_acq_rel);
  }
...
void Process::cleanup() {
  ...
  if (pid > 0 && !impl_->exit_processed.load(std::memory_order_acquire)) {
    ...
    do {
      r = waitpid(static_cast<pid_t>(pid), &status, WNOHANG);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    } while (r < 0 && errno == EINTR);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }

  if (impl_->stdin_pipe[0] >= 0) {
    ::close(impl_->stdin_pipe[0]);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    impl_->stdin_pipe[0] = -1;      // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
  ...
  impl_->process_id.store(-1, std::memory_order_release);
#endif
...
void Process::report_read_result(const ReadResult& result) {
  ...
  if VUNLIKELY (result.read_error) {
    invoke_callbacks_outside_lock(kReadError, false, -1, kNormalExitStatus,  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
                                  impl_->state.load(std::memory_order_acquire), false,
                                  false,   // LCOV_EXCL_LINE GCOVR_EXCL_LINE
                                  false);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }

  if VUNLIKELY (result.truncated) {
    set_error(kBufferOverflowError);
...
bool Process::setup_pipes() {
  ...
  if (mode == kForwardedMode || mode == kForwardedOutputMode || mode == kForwardedErrorMode) {
    ...
    if VUNLIKELY (pipe(impl_->stdin_pipe) != 0) {
      set_error(kStartError);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return false;            // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
...
bool Process::start_program(const std::string& program, const std::vector<std::string>& arguments) {
  ...
  if VUNLIKELY (pipe(exec_failure_pipe) != 0) {
    int err = errno;                                                            // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    CLOG_E("Process: Failed to create exec failure pipe: %s.", strerror(err));  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    return false;                                                               // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
  ...
  impl_->process_id.store(fork_pid, std::memory_order_release);

  if VUNLIKELY (fork_pid < 0) {
    // LCOV_EXCL_START GCOVR_EXCL_START
    int err = errno;
  ...
  if VUNLIKELY (fork_pid == 0) {
    ...
    if VUNLIKELY (!working_directory_copy.empty() && chdir(working_directory_copy.c_str()) != 0) {
      ...
      written = ::write(exec_failure_pipe[1], &error_code, sizeof(error_code));  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  ...
  if (n > 0) {
    ...
    do {
      wait_result = waitpid(fork_pid, &status, 0);
...
void Process::monitor_thread() {
  ...
  while (impl_->monitor_running.load(std::memory_order_acquire) &&
    ...
    if (result == pid) {
      ...
      if VUNLIKELY (errno != EINTR && errno != ECHILD) {  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        break;                                            // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      }
    }
...
void Process::handle_process_exit(int exit_code, ExitStatus status) {
  ...
  if (impl_->exit_processed.compare_exchange_strong(expected, true,
                                                    std::memory_order_acq_rel)) {  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
#ifdef _WIN32
    status =
