...
std::string_view trim_string_view(std::string_view str) noexcept {
  if (str.empty()) {
    return {};
  }
...
std::vector<std::string> split(const std::string& str, char f) noexcept {
  if (str.empty()) {
    return {};
  }
...
std::vector<std::string_view> split_view(std::string_view str, char f) noexcept {
  if (str.empty()) {
    return {};
  }
...
std::vector<std::string> split_any(const std::string& str, std::string_view delimiters) noexcept {
  const auto views = split_any_view(str, delimiters);
...
std::vector<std::string_view> split_any_view(std::string_view str, std::string_view delimiters) noexcept {
  if (str.empty()) {
    return {};
  }

  if (delimiters.empty()) {
    auto token = trim_string_view(str);
  ...
  while ((end = str.find_first_of(delimiters, start)) != std::string_view::npos) {
    if (end > start) {
      auto token = trim_string_view(str.substr(start, end - start));
  ...
  if (start < str.size()) {
    auto token = trim_string_view(str.substr(start));
