...
uint64_t ElapsedTimer::get_cpu_active_time(Accuracy accuracy) noexcept {
  ...
  switch (accuracy) {
    ...
    case kNano:
      return total_time * 1000U;
    default:                      // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return total_time / 1000U;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
#endif
}
...
void ElapsedTimer::start() noexcept {
  ...
  start_time_.compare_exchange_strong(expected, new_value, std::memory_order_acq_rel, std::memory_order_acquire);
}
