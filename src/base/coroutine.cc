 ...
 private:
  ...
  void poll_snapshot(std::vector<MoveFunction<bool()>>&& snapshot) {
...

    {
      ...
      cv_.wait_for(lock, std::chrono::milliseconds(1), [this, generation] {
  ...
  ~WhenAllVoidGuard() {
    ...
    if (state->remaining.fetch_sub(1, std::memory_order_acq_rel) == 1) {
      if VUNLIKELY (state->first_exc) {
        state->promise.set_exception(state->first_exc);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      } else {
...
static void retry_awaiter_resume(MessageLoop* loop, std::shared_ptr<MessageLoop::AliveState> alive_state,
  ...
  detail::register_future_wait(
      MoveFunction<bool()>([loop, alive_state = std::move(alive_state), state = std::move(state),
                            priority,  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        ...
        if VUNLIKELY (++retry_count >= detail::kMaxResumePostRetry) {
          resume_failed_later(state);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
static bool post_callback(MessageLoop& loop, MoveFunction<void()>&& resume_callback,
  ...
  if VUNLIKELY (dropped) {
    state->drop_once();  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  ...
  detail::register_future_wait(MoveFunction<bool()>(
      [loop, alive_state = std::move(alive_state), handle, priority, retry_count = 0U]() mutable -> bool {
        ...
        if VUNLIKELY (!alive_state->alive.load(std::memory_order_acquire) || loop->is_ready_to_quit()) {
          ...
          handle = {};       // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
// DetachedTask
detail::DetachedTask::DetachedTask() noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
void detail::DetachedTask::promise_type::unhandled_exception() noexcept {
  try {
    ...
    CLOG_E("DetachedTask: coroutine threw a non-std exception.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
}
...
detail::ResumePostResult detail::post_callback_if_alive(MessageLoop* loop,
  ...
  if VUNLIKELY (loop == nullptr || !alive_state) {
    if VLIKELY (drop_callback) {  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      drop_callback();            // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }

    return ResumePostResult::kClosed;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
ScheduleAwaiter::ScheduleAwaiter(ScheduleAwaiter&&) noexcept = default;             // LCOV_EXCL_LINE GCOVR_EXCL_LINE
ScheduleAwaiter& ScheduleAwaiter::operator=(ScheduleAwaiter&&) noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
bool ScheduleAwaiter::await_suspend(std::coroutine_handle<> handle) {
  if VUNLIKELY (!state) {
    state = MemoryResource::make_shared<detail::AwaiterResumeState>();  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
// YieldAwaiter
YieldAwaiter::YieldAwaiter() noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

YieldAwaiter::YieldAwaiter(MessageLoop* loop, std::shared_ptr<MessageLoop::AliveState> alive_state,
                           std::shared_ptr<detail::AwaiterResumeState> state, uint16_t priority) noexcept
    : loop(loop), alive_state(std::move(alive_state)), state(std::move(state)), priority(priority) {}

YieldAwaiter::YieldAwaiter(YieldAwaiter&&) noexcept = default;             // LCOV_EXCL_LINE GCOVR_EXCL_LINE
YieldAwaiter& YieldAwaiter::operator=(YieldAwaiter&&) noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
void YieldAwaiter::await_resume() {
  ...
  if VUNLIKELY (failed) {
    throw std::runtime_error("vlink::Coroutine::yield: post_task to loop failed");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
}

// DelayAwaiter
DelayAwaiter::DelayAwaiter() noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
DelayAwaiter::DelayAwaiter(DelayAwaiter&&) noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

DelayAwaiter& DelayAwaiter::operator=(DelayAwaiter&&) noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
void DelayAwaiter::await_resume() {
  ...
  if VUNLIKELY (failed) {
    throw std::runtime_error("vlink::Coroutine::delay_ms: post_task to loop failed");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
}
...
GraphAwaiter::GraphAwaiter(GraphAwaiter&&) noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE

GraphAwaiter& GraphAwaiter::operator=(GraphAwaiter&&) noexcept = default;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
void GraphAwaiter::await_suspend(std::coroutine_handle<> handle) {
  ...
  if VUNLIKELY (state->fired.load(std::memory_order_acquire)) {
    graph->unregister_status_callback(id);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
