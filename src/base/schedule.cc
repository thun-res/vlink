...
// Schedule::Status
Schedule::Status::Status() : impl_(MemoryResource::make_shared<Schedule::Status::Impl>()) {
  impl_->is_valid.store(true, std::memory_order_relaxed);
}

Schedule::Status::~Status() { commit(); }
...
Schedule::Status::Status(Status&& status) noexcept {
  ...
  launcher_ = std::move(status.launcher_);
  committed_ = status.committed_;
}
...
bool Schedule::Status::is_valid() const { return impl_ && impl_->is_valid.load(std::memory_order_relaxed); }

bool Schedule::Status::dispatch() {
  commit();
...
Schedule::Status& Schedule::Status::on_schedule_timeout(Callback&& callback) {
  if VLIKELY (is_valid()) {
    ...
    if VUNLIKELY (impl_->dispatched.load(std::memory_order_acquire)) {
      VLOG_E("Schedule: on_schedule_timeout registered after dispatch; ignored.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return *this;                                                                 // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }

    if VUNLIKELY (impl_->schedule_timeout_callback) {
      VLOG_E("Schedule: Schedule timeout callback is already set.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return *this;                                                   // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
...
Schedule::Status& Schedule::Status::on_catch(CatchCallback&& callback) {
  if VLIKELY (is_valid()) {
    ...
    if VUNLIKELY (impl_->dispatched.load(std::memory_order_acquire)) {
      VLOG_E("Schedule: on_catch registered after dispatch; ignored.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return *this;                                                      // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }

    if VUNLIKELY (impl_->catch_callback) {
      VLOG_E("Schedule: Catch callback is already set.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return *this;                                        // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
...
Schedule::Status& Schedule::RetStatus::on_else(Callback&& callback) {
  if VLIKELY (is_valid()) {
    ...
    if VUNLIKELY (impl_->dispatched.load(std::memory_order_acquire)) {
      VLOG_E("Schedule: on_else registered after dispatch; ignored.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return *this;                                                     // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }

    if VUNLIKELY (impl_->else_callback) {
      VLOG_E("Schedule: Else callback is already set.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return *this;                                       // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
...
Schedule::RetStatus Schedule::internal_process_with_ret(const Config& config, RetCallback&& callback,
  ...
  wrapper_callback = [callback = std::move(callback), config, impl = status.impl_]() mutable {
...

    {
      ...
      if VUNLIKELY (!impl->is_valid.load(std::memory_order_relaxed)) {
        return;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      }
