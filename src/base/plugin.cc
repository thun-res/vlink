...
std::deque<std::string> Plugin::default_search_path() {
  ...
  const std::string plugin_dir_env = Utils::get_env("VLINK_PLUGIN_DIR");
  const std::string local_app_dir = Utils::get_app_dir();
  ...
  std::deque<std::string> search_path{
      ...
      local_app_dir + "/lib64",
      local_app_dir + "/lib",
      "/lib64",  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      "/lib",
  };

  const auto plugin_dirs = Helpers::split_any(plugin_dir_env);

  if (!plugin_dirs.empty()) {
    search_path.insert(search_path.begin(), plugin_dirs.begin(), plugin_dirs.end());
  }
...
Plugin::Handle Plugin::load_and_create(const std::string& plugin_id, const std::string& lib_name,
  ...
  try {
    ...
    if VUNLIKELY (!inserted) {
      destroy(entry, handle);  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return nullptr;          // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
