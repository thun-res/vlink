...
Bytes Bytes::from_user_input(const std::string& str, bool* ok) noexcept {
  ...
  auto parse_byte = [&vec, ok](std::string_view token) -> bool {
    if (token.empty()) {
      if (ok) {       // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        *ok = false;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      }
...
Bytes Bytes::reverse_order(const Bytes& target) noexcept {
  ...
  if VUNLIKELY (result.empty()) {
    return result;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
bool Bytes::operator==(const Bytes& target) const noexcept {
  ...
  if (!this_data || !target_data) {
    return size_ == 0;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
Bytes Bytes::compress_data(const uint8_t* data, size_t size, bool high_ratio) noexcept {
  ...
  if VUNLIKELY (result <= 0) {
    target_bytes.clear();  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    return target_bytes;   // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
bool Bytes::shrink_to(size_t size) noexcept {
  ...
  if VUNLIKELY (size > capacity_) {
    VLOG_E("Bytes: Cannot shrink_to a size larger than capacity.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    return false;                                                    // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
...
bool Bytes::reserve(size_t new_capacity) noexcept {
  ...
  if (total_new_size > kStackSize) {
    ...
    if VUNLIKELY (!new_data) {
      VLOG_E("Bytes: Failed to reserve memory.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return false;                                // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
  ...
  switch (type) {
    case kCreate: {
      ...
      if (total_size > kStackSize) {
        ...
        if VUNLIKELY (!new_data) {
          VLOG_E("Bytes: Failed to allocate memory.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      ...
      if (is_owner_ && data_) {
        if (data_ != stack_data_ && capacity_ + offset_ > kStackSize) {  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
          bytes_free(data_, capacity_ + offset_);                        // LCOV_EXCL_LINE GCOVR_EXCL_LINE
          data_ = nullptr;                                               // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        }
      }
      ...
      if (is_owner_ && data_) {
        ...
        if (can_reuse && data) {
          ...
          if (src_start < dst_end && dst_start < src_end) {
            can_reuse = false;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
          }
        }
    ...
    case kMove: {
      ...
      if VUNLIKELY (!tmp) {
        VLOG_E("Bytes: Reference tmp is empty.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
