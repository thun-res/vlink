...
bool SysSemaphore::attach(const std::string& name) {
  ...
  if VUNLIKELY (impl_->handle == SEM_FAILED) {
    VLOG_E("SysSemaphore: sem_open failed.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
bool SysSemaphore::detach(bool force) {
  ...
  if VUNLIKELY (impl_->handle == SEM_FAILED) {
    VLOG_E("SysSemaphore: Handle is empty.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    return false;                              // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
  ...
  if VUNLIKELY (::sem_close(impl_->handle) == -1) {
    VLOG_E("SysSemaphore: sem_close failed.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  ...
  if (force && !impl_->name.empty()) {
    if VUNLIKELY (::sem_unlink(impl_->name.c_str()) == -1 && errno != ENOENT) {
      VLOG_E("SysSemaphore: sem_unlink failed.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      ok = false;                                  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
    }
  }
...
bool SysSemaphore::acquire(size_t n, int timeout_ms) {
  ...
  if (timeout_ms < 0) {
    ...
    if VUNLIKELY (::clock_gettime(CLOCK_REALTIME, &ts) == -1) {
      VLOG_E("SysSemaphore: clock_gettime failed.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      return false;
    }
...
void SysSemaphore::release(size_t n) {
  ...
  for (; n > 0; --n) {
    if VUNLIKELY (::sem_post(impl_->handle) == -1) {
      VLOG_E("SysSemaphore: sem_post failed.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
size_t SysSemaphore::get_count() const {
  ...
  if VUNLIKELY (::sem_getvalue(impl_->handle, &count) != 0) {
    VLOG_E("SysSemaphore: sem_getvalue failed.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
