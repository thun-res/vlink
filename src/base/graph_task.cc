...
void GraphTask::cancel() {
  ...
  while (!task_stack.empty()) {
...

    {
      ...
      if (current_task->impl_->status.load(std::memory_order_acquire) == kStatusInActive) {
        continue;
      }
...
uint32_t GraphTask::register_status_callback(StatusCallback&& callback) {
  ...
  for (uint32_t attempts = 0; attempts < std::numeric_limits<uint32_t>::max();
       ++attempts) {  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  ...
  if VUNLIKELY (id == 0) {
    VLOG_E("GraphTask: status_callback id space exhausted.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
...
void GraphTask::set_condition_number(int condition_number) {
  impl_->condition_number.store(condition_number, std::memory_order_relaxed);
}

void GraphTask::set_priority(uint16_t priority) { impl_->priority.store(priority, std::memory_order_relaxed); }

void GraphTask::set_max_recursion_depth(uint32_t depth) {
  impl_->max_recursion_depth.store(depth, std::memory_order_relaxed);
}

void GraphTask::set_policy(Policy policy) { impl_->policy.store(policy, std::memory_order_relaxed); }
...
int GraphTask::get_condition_number() const { return impl_->condition_number.load(std::memory_order_relaxed); }

uint16_t GraphTask::get_priority() const { return impl_->priority.load(std::memory_order_relaxed); }

uint32_t GraphTask::get_max_recursion_depth() const {
  return impl_->max_recursion_depth.load(std::memory_order_relaxed);
}

GraphTask::Policy GraphTask::get_policy() const { return impl_->policy.load(std::memory_order_relaxed); }
...
void GraphTask::remove_precede_task(const std::shared_ptr<GraphTask>& task) {
  ...
  if VLIKELY (iter_precede != task->impl_->precede_task_list.end()) {
    ...
    VLOG_F("GraphTask: Current task not found in task's precede_task_list.");  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
}
...
std::string GraphTask::export_to_dot() const {
  ...
  for (const auto& [group_name, tasks] : groups) {
    ...
    for (const auto* task : tasks) {
      ...
      } else if (task->impl_->policy.load(std::memory_order_relaxed) == kPolicyMultiple) {
        extra_label = "\\n[Multiple]";
        color_label = "lightgreen";
      } else if (task->impl_->policy.load(std::memory_order_relaxed) ==
                 kPolicyWaitAll) {  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        extra_label = "\\n[Waitfor]";
        color_label = "lightblue";
      } else {
        extra_label.clear();        // LCOV_EXCL_LINE GCOVR_EXCL_LINE
        color_label = "lightgray";  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
      }
...
void GraphTask::update_status(Status status) {
  if VUNLIKELY (impl_->status.load(std::memory_order_acquire) == status) {
    return;  // LCOV_EXCL_LINE GCOVR_EXCL_LINE
  }
