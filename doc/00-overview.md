...
VLink 是面向自动驾驶与具身智能的高性能通信中间件，定位为 ROS 2 的全场景替代方案。其核心设计目标是：用一套类型安全的统一 API，覆盖进程内、共享内存、车载以太网与跨机网络的全部通信需求，并使通信后端的更换退化为一次 URL 前缀的修改——业务代码不随之改动。
...
依 [0.2 URL 即通信契约](#-02-核心抽象url-即通信契约)，仅替换 `intra://` 前缀即可使上述代码跨进程或跨机器运行，业务逻辑不变。完整可运行示例见 [快速开始](01-started.md) 与 `examples/quickstart/hello_pubsub/`。
...
**录制与回放**　节点级 `set_record_path()` 自动录制经过的消息；`.vdb` 格式紧凑，`.vcap`（MCAP）可由 Foxglove 直接打开。详见 [录制与回放](09-recording.md)。

**服务发现**　基于 UDP 组播自动感知系统中全部活跃节点、话题与传输类型，无需中心注册。详见 [可观测性](12-observability.md)。
...
延迟特征由后端决定：`intra://` 接近一次函数调用；`shm://` 经 Iceoryx 零拷贝将大负载传递降至指针级；跨机 `dds://` 与原生 DDS 相当。量化基准与复现方法见 [测试与贡献](15-contributing.md)。
