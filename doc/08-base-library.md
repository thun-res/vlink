...
`VLINK_LOG_PID_DIR=1` 时再追加 `<PID>` 子目录供同名多实例并存，轮转上限按该子目录
...
`VLINK_LOG_PID_DIR=1`。`flush()` 刷新标准输出、标准错误与内置文件后端/插件；自定义 handler 自行管理缓冲。
文件 flush 仅把 C 运行库缓冲提交给操作系统，不等同于断电持久化所需的 `fsync`。

handler 在锁外执行，可在回调内替换或注销；已开始的调用可在注销返回后结束，捕获对象须
保持有效，handler 须支持并发调用。backtrace 仅改变内置文件后端：所有等级进入环，
