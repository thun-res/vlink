...
// WriterListener
DdsSetterImpl::WriterListener::WriterListener(NodeImpl* impl) : DdsWriterListener(impl) {}

// DdsSetterImpl
DdsSetterImpl::DdsSetterImpl(const DdsConf& conf) : conf_(conf) {}

void DdsSetterImpl::init() {
...
const Conf* DdsSetterImpl::get_conf() const { return &conf_; }
...
void DdsSetterImpl::write(const Bytes& msg_data) {
  ...
  DdsFactory::write_data(writer_.get(), msg_data, seq_++);
}
