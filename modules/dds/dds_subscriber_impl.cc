...
void DdsSubscriberImpl::ReaderListener::on_data_available(dds::DataReader* reader) {
  ...
  if VUNLIKELY (instance->has_suspend.load(std::memory_order_relaxed)) {
...
void DdsSubscriberImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_) {
    VLOG_E("DdsSubscriberImpl::init(): participant/topic creation failed; subscriber left uninitialised.");
...
bool DdsSubscriberImpl::suspend() {
  has_suspend.store(true, std::memory_order_relaxed);

  return true;
}

bool DdsSubscriberImpl::resume() {
  has_suspend.store(false, std::memory_order_relaxed);

  return true;
}

bool DdsSubscriberImpl::is_suspend() const { return has_suspend.load(std::memory_order_relaxed); }
