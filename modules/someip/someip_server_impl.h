...
// SomeipServerImpl
class SomeipServerImpl final : public ServerImpl {
 public:
  using Object = SomeipServer;
  static constexpr uint8_t kImplType = kServer | kPublisher | kSetter;

  explicit SomeipServerImpl(const SomeipConf& conf);
 ...
 private:
  ...
  VLINK_DISALLOW_COPY_AND_ASSIGN(SomeipServerImpl)
};
