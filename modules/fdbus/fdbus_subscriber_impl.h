...
// FdbusSubscriberImpl
class FdbusSubscriberImpl final : public SubscriberImpl {
 public:
  using Object = FdbusClient;
  static constexpr uint8_t kImplType = kClient | kSubscriber | kGetter;

  explicit FdbusSubscriberImpl(const FdbusConf& conf);
 ...
 private:
  ...
  VLINK_DISALLOW_COPY_AND_ASSIGN(FdbusSubscriberImpl)
};
