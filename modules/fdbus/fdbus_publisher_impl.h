...
// FdbusPublisherImpl
class FdbusPublisherImpl final : public PublisherImpl {
 public:
  using Object = FdbusServer;
  static constexpr uint8_t kImplType = kServer | kPublisher | kSetter;

  explicit FdbusPublisherImpl(const FdbusConf& conf);
 ...
 private:
  ...
  VLINK_DISALLOW_COPY_AND_ASSIGN(FdbusPublisherImpl)
};
