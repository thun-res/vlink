...
QnxServer::QnxServer(const QnxID& id) {
  ...
  do {
    fd_.store(::name_attach(nullptr, address_.c_str(), 0), std::memory_order_release);

    if VUNLIKELY (!fd_.load(std::memory_order_acquire)) {
      std::this_thread::sleep_for(std::chrono::microseconds(10));
      if (++retry_cnt > 3) {
        break;
      }
    }
  } while (!fd_.load(std::memory_order_acquire) && !quit_flag_.load(std::memory_order_acquire));

  if VUNLIKELY (!fd_.load(std::memory_order_acquire) && !quit_flag_.load(std::memory_order_acquire)) {
...
QnxServer::~QnxServer() {
  ...
  if VUNLIKELY (is_busy_.load(std::memory_order_acquire)) {
    QnxFactory::get_message_loop().wait_for_idle();
  }

  auto* fd = fd_.load(std::memory_order_acquire);
  wake_channel(fd);
  ...
  fd = fd_.exchange(nullptr, std::memory_order_acq_rel);
  ...
  while (!quit_flag_.load(std::memory_order_acquire)) {
    ...
    if (rcvid == 0) {
      if (header.pulse.code == _PULSE_CODE_CUSTOM_CONNECT) {
        ...
        } while (back_coid < 0 && !quit_flag_.load(std::memory_order_acquire));

        if VUNLIKELY (back_coid < 0 && !quit_flag_.load(std::memory_order_acquire)) {
...
bool QnxClient::call(uint32_t channel, const Bytes& req_data, NodeImpl::MsgCallback&& callback) {
  if VUNLIKELY (coid_.load(std::memory_order_acquire) < 0 || !back_fd_.load(std::memory_order_acquire)) {
    return false;
  }

  is_busy_.store(true, std::memory_order_release);

  QnxFactory::get_message_loop().post_task([weak_self = weak_from_this(), channel, callback = std::move(callback),
                                            req_data] {
    auto self = weak_self.lock();
    ...
    if VUNLIKELY (self->coid_.load(std::memory_order_acquire) < 0 || !self->back_fd_.load(std::memory_order_acquire)) {
      self->is_busy_.store(false, std::memory_order_release);
      return;
    }
    ...
    if VLIKELY (callback) {
      ...
      ret = ::MsgSend(self->coid_.load(std::memory_order_acquire), req_data_buf.data(), req_data_buf.size(), &resp_msg,
                      sizeof(resp_msg));
      ...
      if (resp_msg.size <= QnxMsg::kBufferSize) {
        ...
        ::MsgSend(self->coid_.load(std::memory_order_acquire), &header, sizeof(header), resp_data.data(),
                  resp_data.size());

        callback(resp_data);
      }
    } else {
      ret = ::MsgSend(self->coid_.load(std::memory_order_acquire), req_data_buf.data(), req_data_buf.size(), 0, 0);
...
bool QnxClient::subscribe(uint32_t channel) {
  ...
  QnxFactory::get_message_loop().post_task([weak_self = weak_from_this(), channel] {
    ...
    header.msg.other.id = self->coid_.load(std::memory_order_acquire) | QnxFactory::get_pid();

    ret = ::MsgSend(self->coid_.load(std::memory_order_acquire), &header, sizeof(header), 0, 0);
...
bool QnxClient::is_connected() const { return coid_.load(std::memory_order_acquire) > 0; }

bool QnxClient::listen() {
  back_fd_.store(
      ::name_attach(
          nullptr,
          QnxFactory::get_back_address(address_, coid_.load(std::memory_order_acquire) | QnxFactory::get_pid()).c_str(),
          0),
      std::memory_order_release);

  if VUNLIKELY (!back_fd_.load(std::memory_order_acquire)) {
  ...
  thread_ = std::thread([this] {
    ...
    while (!quit_flag_.load(std::memory_order_acquire) && coid_.load(std::memory_order_acquire) >= 0) {
      auto* back_fd = back_fd_.load(std::memory_order_acquire);
      ...
      if (rcvid == 0) {
        if (header.pulse.code == _PULSE_CODE_DISCONNECT) {
          if VLIKELY (!quit_flag_.load(std::memory_order_acquire)) {
            is_busy_.store(true, std::memory_order_release);
      ...
      if (header.msg.type == QnxMsg::kPublish) {
        ...
        if VUNLIKELY (size != static_cast<ssize_t>(msg_data.size()) && !quit_flag_.load(std::memory_order_acquire)) {
...
void QnxClient::start_timer() {
  ...
  timer_.start([weak_self] {
    ...
    if (self->coid_.load(std::memory_order_acquire) < 0 || !self->back_fd_.load(std::memory_order_acquire)) {
...
void QnxClient::try_connect() {
  ...
  if VLIKELY (coid_.load(std::memory_order_acquire) < 0) {
    coid_.store(::name_open(address_.c_str(), 0), std::memory_order_release);
    if VUNLIKELY (coid_.load(std::memory_order_acquire) < 0) {
      return;
    }
  }

  if (!back_fd_.load(std::memory_order_acquire)) {
    if VUNLIKELY (!listen()) {
      disconnect();
      return;
    }
  ...
  do {
    ret = ::MsgSendPulse(coid_.load(std::memory_order_acquire), SIGEV_PULSE_PRIO_INHERIT, _PULSE_CODE_CUSTOM_CONNECT,
                         coid_.load(std::memory_order_acquire) | QnxFactory::get_pid());
  ...
  } while (ret < 0 && !quit_flag_.load(std::memory_order_acquire));

  if VUNLIKELY (ret < 0 && !quit_flag_.load(std::memory_order_acquire)) {
...
void QnxClient::try_detect() {
  ...
  do {
    ret = ::MsgSendPulse(coid_.load(std::memory_order_acquire), SIGEV_PULSE_PRIO_INHERIT, _PULSE_CODE_CUSTOM_HEARTBEAT,
                         0);
...
void QnxClient::disconnect() {
  int coid = coid_.exchange(-1, std::memory_order_acq_rel);
  ...
  back_fd = back_fd_.exchange(nullptr, std::memory_order_acq_rel);
