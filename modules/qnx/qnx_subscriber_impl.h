...
// QnxSubscriberImpl
class QnxSubscriberImpl final : public SubscriberImpl {
 public:
  using Object = QnxClient;
  static constexpr uint8_t kImplType = kClient | kSubscriber | kGetter;

  explicit QnxSubscriberImpl(const QnxConf& conf);
 ...
 private:
  ...
  VLINK_DISALLOW_COPY_AND_ASSIGN(QnxSubscriberImpl)
};
