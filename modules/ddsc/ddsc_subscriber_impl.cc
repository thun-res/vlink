...
void DdscSubscriberImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_) {
    VLOG_E("DdscSubscriberImpl::init(): participant/topic creation failed; subscriber left uninitialised.");
...
bool DdscSubscriberImpl::is_suspend() const { return has_suspend.load(std::memory_order_acquire); }
