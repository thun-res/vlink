...
void DdstGetterImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_) {
    VLOG_E("DdstGetterImpl::init(): participant/topic creation failed; getter left uninitialised.");
...
bool DdstGetterImpl::is_suspend() const { return has_suspend.load(std::memory_order_relaxed); }
