...
void DdstServerImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_req_) {
    VLOG_E("DdstServerImpl::init(): participant/topic creation failed; server left uninitialised.");
...
bool DdstServerImpl::is_suspend() const { return has_suspend.load(std::memory_order_acquire); }
...
bool DdstServerImpl::has_clients() const { return read_session_count_.load(std::memory_order_relaxed) > 0; }
