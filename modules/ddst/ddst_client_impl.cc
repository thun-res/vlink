...
void DdstClientImpl::init() {
  ...
  if VUNLIKELY (!participant_) {
    VLOG_E("DdstClientImpl::init(): participant creation failed; client left uninitialised.");
...
bool DdstClientImpl::call(const Bytes& req_data, MsgCallback&& callback, std::chrono::milliseconds timeout) {
  ...
  uint64_t id = DdstFactory::get_guid(writer_->get_guid(), seq_.fetch_add(1, std::memory_order_relaxed) + 1);
