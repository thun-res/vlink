...
void DdstPublisherImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_) {
    VLOG_E("DdstPublisherImpl::init(): participant/topic creation failed; publisher left uninitialised.");
...
bool DdstPublisherImpl::has_subscribers() const { return session_count_.load(std::memory_order_acquire) > 0; }

bool DdstPublisherImpl::write(const Bytes& msg_data) {
  return DdstFactory::write_data(writer_.get(), msg_data, seq_.fetch_add(1, std::memory_order_relaxed));
}
