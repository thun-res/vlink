...
void DdstSubscriberImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_) {
    VLOG_E("DdstSubscriberImpl::init(): participant/topic creation failed; subscriber left uninitialised.");
...
bool DdstSubscriberImpl::is_suspend() const { return has_suspend.load(std::memory_order_acquire); }
