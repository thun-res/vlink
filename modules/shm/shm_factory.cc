...
ShmFactory::ShmFactory() {
  ...
  global_instance.has_factory_inited.store(true, std::memory_order_release);
  ...
  detect_timer_.set_callback([this]() {
    if VUNLIKELY (!global_instance.has_runtime_inited.load(std::memory_order_acquire)) {
      return;
    }
...

    {
      ...
      callbacks.reserve(detect_map_.size());
...
bool ShmFactory::has_roudi_inited() { return ShmGlobal::get().has_roudi_inited.load(std::memory_order_acquire); }

bool ShmFactory::has_runtime_inited() { return ShmGlobal::get().has_runtime_inited.load(std::memory_order_acquire); }
...
void ShmFactory::init_roudi(const std::string& config_path, int memory_strategy, bool monitoring_enable) {
  ...
  if VUNLIKELY (!ShmGlobal::get().has_roudi_inited.compare_exchange_strong(expected, true, std::memory_order_acq_rel,
...
void ShmFactory::init_runtime(std::string name, bool same_process_from_roudi) {
  ...
  if VLIKELY (global_instance.has_runtime_inited.compare_exchange_strong(expected, true, std::memory_order_acq_rel,
                                                                         std::memory_order_relaxed)) {
    if (name.empty()) {
...
void ShmFactory::deinit_runtime() {
  ...
  if VLIKELY (global_instance.has_runtime_inited.compare_exchange_strong(expected, false, std::memory_order_acq_rel,
                                                                         std::memory_order_relaxed)) {
    if (global_instance.has_factory_inited.load(std::memory_order_acquire)) {
...
void ShmFactory::deinit_roudi() {
  ...
  if VLIKELY (global_instance.has_roudi_inited.compare_exchange_strong(expected, false, std::memory_order_acq_rel,
                                                                       std::memory_order_relaxed)) {
...
bool ShmServer::is_suspend() const { return is_suspend_.load(std::memory_order_relaxed); }

void ShmServer::process_message() {
  while (server_->hasRequests() && !quit_flag_.load(std::memory_order_acquire)) {
    server_->take()
...
Bytes ShmServer::loan(uint64_t channel, int64_t size) {
  ...
  ShmFactory::write_header(write_req, channel, seq_.load(std::memory_order_relaxed));
...
bool ShmServer::reply(uint64_t channel, const Bytes& resp_data) {
  if (resp_data.is_loaned()) {
    ...
    ShmFactory::write_data(write_resp, channel, seq_.load(std::memory_order_relaxed), resp_data);
...
void ShmServer::on_request_received(shm::popo::UntypedServer*, ShmServer* target) {
  std::lock_guard callback_lock(target->callback_mtx_);

  if VUNLIKELY (target->quit_flag_.load(std::memory_order_acquire)) {
    return;
  }

  if VUNLIKELY (target->is_suspend_.load(std::memory_order_relaxed)) {
...
bool ShmClient::call(uint64_t channel, const Bytes& req_data, NodeImpl::MsgCallback&& callback, uint64_t* seq_out) {
  ...
  if (req_data.is_loaned()) {
    ...
    ShmFactory::write_data(write_req, channel, seq_.load(std::memory_order_relaxed), req_data);
...
Bytes ShmPublisher::loan(uint64_t channel, int64_t size) {
  ...
  ShmFactory::write_header(write_msg, channel, seq_.load(std::memory_order_relaxed));
...
void ShmPublisher::discovery_subscribers(bool has_subscribers) {
  if VLIKELY (last_has_subscribers_.load(std::memory_order_relaxed) == has_subscribers) {
    return;
  }

  traverse_sub_connect_callback([has_subscribers](NodeImpl*, const auto& callback) { callback(has_subscribers); });
  last_has_subscribers_.store(has_subscribers, std::memory_order_relaxed);
}
