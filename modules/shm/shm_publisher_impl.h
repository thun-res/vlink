...
// ShmPublisherImpl
class ShmPublisherImpl final : public PublisherImpl {
 public:
  using Object = ShmPublisher;
  static constexpr uint8_t kImplType = kPublisher | kSetter;

  explicit ShmPublisherImpl(const ShmConf& conf);
 ...
 private:
  ...
  void detect_subscribers(ConnectCallback&& callback) override;

  bool wait_for_subscribers(std::chrono::milliseconds timeout) override;
  ...
  VLINK_DISALLOW_COPY_AND_ASSIGN(ShmPublisherImpl)
};
