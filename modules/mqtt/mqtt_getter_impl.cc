...
// MqttGetterImpl
MqttGetterImpl::MqttGetterImpl(const MqttConf& conf) : conf_(conf) {}

void MqttGetterImpl::init() {
...
const AbstractNode* MqttGetterImpl::get_abstract_node() const { return object_.get(); }

bool MqttGetterImpl::listen(MsgCallback&& callback) {
...
void MqttGetterImpl::set_latency_and_lost_enabled(bool enable) {
...
bool MqttGetterImpl::is_latency_and_lost_enabled() const {
  if (object_) {
...
int64_t MqttGetterImpl::get_latency() const {
  if (object_) {
...
SampleLostInfo MqttGetterImpl::get_lost() const {
