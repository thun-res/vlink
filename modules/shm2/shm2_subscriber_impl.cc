...
// Shm2SubscriberImpl
Shm2SubscriberImpl::Shm2SubscriberImpl(const Shm2Conf& conf) : conf_(conf) {}

void Shm2SubscriberImpl::init() {
...
bool Shm2SubscriberImpl::return_loan(const Bytes& bytes) { return object_->release(bytes); }

const Conf* Shm2SubscriberImpl::get_conf() const { return &conf_; }

const AbstractNode* Shm2SubscriberImpl::get_abstract_node() const { return object_.get(); }

bool Shm2SubscriberImpl::listen(MsgCallback&& callback) {
...
void Shm2SubscriberImpl::set_latency_and_lost_enabled(bool enable) {
...
bool Shm2SubscriberImpl::is_latency_and_lost_enabled() const {
  if (object_) {
...
SampleLostInfo Shm2SubscriberImpl::get_lost() const {
