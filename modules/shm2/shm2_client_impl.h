...
// Shm2ClientImpl
class Shm2ClientImpl final : public ClientImpl, public AbstractNode {
 public:
  using Object = Shm2Client;
  static constexpr uint8_t kImplType = kClient;

  explicit Shm2ClientImpl(const Shm2Conf& conf);
 ...
 private:
  ...
  void detect_connected(ConnectCallback&& callback) override;

  bool wait_for_connected(std::chrono::milliseconds timeout) override;
  ...
  VLINK_DISALLOW_COPY_AND_ASSIGN(Shm2ClientImpl)
};
