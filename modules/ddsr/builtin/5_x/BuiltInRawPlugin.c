...
void vlink_BuiltInRawPluginSupport_print_data(const vlink_BuiltInRaw *sample, const char *desc,
  ...
  if (DDS_OctetSeq_get_contiguous_bufferI(&sample->data) != NULL) {
    RTICdrType_printArray(DDS_OctetSeq_get_contiguous_bufferI(&sample->data), DDS_OctetSeq_get_length(&sample->data),
                          RTI_CDR_OCTET_SIZE, (RTICdrTypePrintFunction)RTICdrType_printOctet, "data", indent_level + 1);
  } else {
    RTICdrType_printPointerArray(DDS_OctetSeq_get_discontiguous_bufferI(&sample->data),
                                 DDS_OctetSeq_get_length(&sample->data), (RTICdrTypePrintFunction)RTICdrType_printOctet,
                                 "data", indent_level + 1);
  }
}
...
PRESTypePluginEndpointData vlink_BuiltInRawPlugin_on_endpoint_attached(
  ...
  if (endpoint_info->endpointKind == PRES_TYPEPLUGIN_ENDPOINT_WRITER) {
    ...
    if (PRESTypePluginDefaultEndpointData_createWriterPool(
            ...
            epd, (PRESTypePluginGetSerializedSampleSizeFunction)vlink_BuiltInRawPlugin_get_serialized_sample_size,
            epd) == RTI_FALSE) {
...
RTIBool vlink_BuiltInRawPlugin_serialize(PRESTypePluginEndpointData endpoint_data, const vlink_BuiltInRaw *sample,
                                            struct RTICdrStream *stream, RTIBool serialize_encapsulation,
                                            RTIEncapsulationId encapsulation_id, RTIBool serialize_sample,
  ...
  if (serialize_sample) {
    if (!RTICdrStream_serializeUnsignedLongLong(stream, &sample->id)) {
      return RTI_FALSE;
    }

    if (DDS_OctetSeq_get_contiguous_bufferI(&sample->data) != NULL) {
      if (!RTICdrStream_serializePrimitiveSequence(stream, DDS_OctetSeq_get_contiguous_bufferI(&sample->data),
                                                   DDS_OctetSeq_get_length(&sample->data), (RTI_INT32_MAX - 1),
                                                   RTI_CDR_OCTET_TYPE)) {
        return RTI_FALSE;
      }
    } else {
      if (!RTICdrStream_serializePrimitivePointerSequence(
              stream, (const void **)DDS_OctetSeq_get_discontiguous_bufferI(&sample->data),
              DDS_OctetSeq_get_length(&sample->data), (RTI_INT32_MAX - 1), RTI_CDR_OCTET_TYPE)) {
        return RTI_FALSE;
      }
    }
  }
...
RTIBool vlink_BuiltInRawPlugin_deserialize_sample(PRESTypePluginEndpointData endpoint_data,
                                                     vlink_BuiltInRaw *sample, struct RTICdrStream *stream,
                                                     RTIBool deserialize_encapsulation, RTIBool deserialize_sample,
                                                     void *endpoint_plugin_qos) {
  char *position = NULL;
  ...
  if (deserialize_sample) {
    vlink_BuiltInRaw_initialize_ex(sample, RTI_FALSE, RTI_FALSE);

    if (!RTICdrStream_deserializeUnsignedLongLong(stream, &sample->id)) {
      goto fin;
    }
    {
      ...
      if (!DDS_OctetSeq_set_maximum(&sample->data, sequence_length)) {
        return RTI_FALSE;
      }
      if (DDS_OctetSeq_get_contiguous_bufferI(&sample->data) != NULL) {
        if (!RTICdrStream_deserializePrimitiveSequence(stream, DDS_OctetSeq_get_contiguous_bufferI(&sample->data),
                                                       &sequence_length, DDS_OctetSeq_get_maximum(&sample->data),
                                                       RTI_CDR_OCTET_TYPE)) {
          goto fin;
        }
      } else {
        if (!RTICdrStream_deserializePrimitivePointerSequence(
                stream, (void **)DDS_OctetSeq_get_discontiguous_bufferI(&sample->data), &sequence_length,
                DDS_OctetSeq_get_maximum(&sample->data), RTI_CDR_OCTET_TYPE)) {
          goto fin;
        }
      }
...
fin:
  if (done != RTI_TRUE && RTICdrStream_getRemainder(stream) >= RTI_CDR_PARAMETER_HEADER_ALIGNMENT) {
    return RTI_FALSE;
  }
  if (deserialize_encapsulation) {
    RTICdrStream_restoreAlignment(stream, position);
...
RTIBool vlink_BuiltInRawPlugin_serialize_to_cdr_buffer(char *buffer, unsigned int *length,
  ...
  if (buffer == NULL) {
    *length = vlink_BuiltInRawPlugin_get_serialized_sample_size(
        (PRESTypePluginEndpointData)&epd, RTI_TRUE, RTICdrEncapsulation_getNativeCdrEncapsulationId(), 0, sample);
  ...
  result = vlink_BuiltInRawPlugin_serialize((PRESTypePluginEndpointData)&epd, sample, &stream, RTI_TRUE,
                                               RTICdrEncapsulation_getNativeCdrEncapsulationId(), RTI_TRUE, NULL);
...
RTIBool vlink_BuiltInRawPlugin_deserialize_from_cdr_buffer(vlink_BuiltInRaw *sample, const char *buffer,
  ...
  vlink_BuiltInRaw_finalize_optional_members(sample, RTI_TRUE);
  return vlink_BuiltInRawPlugin_deserialize_sample(NULL, sample, &stream, RTI_TRUE, RTI_TRUE, NULL);
}

DDS_ReturnCode_t vlink_BuiltInRawPlugin_data_to_string(const vlink_BuiltInRaw *sample, char *str,
  ...
  retCode = DDS_DynamicDataFormatter_to_string_w_format(data, str, str_size, &printFormat);
  if (retCode != DDS_RETCODE_OK) {
    RTIOsapiHeap_freeBuffer(buffer);
    DDS_DynamicData_delete(data);
    return retCode;
...
RTIBool vlink_BuiltInRawPlugin_deserialize(PRESTypePluginEndpointData endpoint_data, vlink_BuiltInRaw **sample,
                                              RTIBool *drop_sample, struct RTICdrStream *stream,
                                              RTIBool deserialize_encapsulation, RTIBool deserialize_sample,
                                              void *endpoint_plugin_qos) {
  RTIBool result;
  const char *METHOD_NAME = "vlink_BuiltInRawPlugin_deserialize";
  if (drop_sample) {
  } /* To avoid warnings */

  stream->_xTypesState.unassignable = RTI_FALSE;
  result =
      vlink_BuiltInRawPlugin_deserialize_sample(endpoint_data, (sample != NULL) ? *sample : NULL, stream,
                                                   deserialize_encapsulation, deserialize_sample, endpoint_plugin_qos);
  if (result) {
  ...
  if (!result && stream->_xTypesState.unassignable) {
    RTICdrLog_exception(METHOD_NAME, &RTI_CDR_LOG_UNASSIGNABLE_SAMPLE_OF_TYPE_s, "vlink_BuiltInRaw");
  }

  return result;
}

RTIBool vlink_BuiltInRawPlugin_skip(PRESTypePluginEndpointData endpoint_data, struct RTICdrStream *stream,
                                       RTIBool skip_encapsulation, RTIBool skip_sample, void *endpoint_plugin_qos) {
  char *position = NULL;
  ...
  if (skip_sample) {
    ...
    {
      RTICdrUnsignedLong sequence_length;
      if (!RTICdrStream_skipPrimitiveSequence(stream, &sequence_length, RTI_CDR_OCTET_TYPE)) {
        goto fin;
      }
    }
  }
...
unsigned int vlink_BuiltInRawPlugin_get_serialized_sample_max_size_ex(PRESTypePluginEndpointData endpoint_data,
                                                                         RTIBool *overflow,
                                                                         RTIBool include_encapsulation,
                                                                         RTIEncapsulationId encapsulation_id,
                                                                         unsigned int current_alignment) {
...
unsigned int vlink_BuiltInRawPlugin_get_serialized_sample_max_size(PRESTypePluginEndpointData endpoint_data,
  ...
  size = vlink_BuiltInRawPlugin_get_serialized_sample_max_size_ex(endpoint_data, &overflow, include_encapsulation,
...
unsigned int vlink_BuiltInRawPlugin_get_serialized_sample_min_size(PRESTypePluginEndpointData endpoint_data,
                                                                      RTIBool include_encapsulation,
                                                                      RTIEncapsulationId encapsulation_id,
                                                                      unsigned int current_alignment) {
  ...
  current_alignment += RTICdrType_getLongLongMaxSizeSerialized(current_alignment);
  current_alignment += RTICdrType_getPrimitiveSequenceMaxSizeSerialized(current_alignment, 0, RTI_CDR_OCTET_TYPE);
 ...
 * encapsulation flags.
 */
unsigned int vlink_BuiltInRawPlugin_get_serialized_sample_size(PRESTypePluginEndpointData endpoint_data,
                                                                  RTIBool include_encapsulation,
                                                                  RTIEncapsulationId encapsulation_id,
                                                                  unsigned int current_alignment,
                                                                  const vlink_BuiltInRaw *sample) {
  ...
  current_alignment += RTICdrType_getPrimitiveSequenceSerializedSize(
      PRESTypePluginDefaultEndpointData_getAlignment(endpoint_data, current_alignment),
      DDS_OctetSeq_get_length(&sample->data), RTI_CDR_OCTET_TYPE);
...
RTIBool vlink_BuiltInRawPlugin_serialize_key(PRESTypePluginEndpointData endpoint_data,
                                                const vlink_BuiltInRaw *sample, struct RTICdrStream *stream,
                                                RTIBool serialize_encapsulation, RTIEncapsulationId encapsulation_id,
                                                RTIBool serialize_key, void *endpoint_plugin_qos) {
  char *position = NULL;
  ...
  if (serialize_key) {
    if (!vlink_BuiltInRawPlugin_serialize(endpoint_data, sample, stream, RTI_FALSE, encapsulation_id, RTI_TRUE,
                                             endpoint_plugin_qos)) {
      return RTI_FALSE;
    }
  }
...
RTIBool vlink_BuiltInRawPlugin_deserialize_key_sample(PRESTypePluginEndpointData endpoint_data,
                                                         vlink_BuiltInRaw *sample, struct RTICdrStream *stream,
                                                         RTIBool deserialize_encapsulation, RTIBool deserialize_key,
                                                         void *endpoint_plugin_qos) {
  char *position = NULL;
  ...
  if (deserialize_key) {
    if (!vlink_BuiltInRawPlugin_deserialize_sample(endpoint_data, sample, stream, RTI_FALSE, RTI_TRUE,
                                                      endpoint_plugin_qos)) {
      return RTI_FALSE;
    }
  }
...
RTIBool vlink_BuiltInRawPlugin_deserialize_key(PRESTypePluginEndpointData endpoint_data,
                                                  ...
                                                  struct RTICdrStream *stream, RTIBool deserialize_encapsulation,
  ...
  stream->_xTypesState.unassignable = RTI_FALSE;
  result =
      vlink_BuiltInRawPlugin_deserialize_key_sample(endpoint_data, (sample != NULL) ? *sample : NULL, stream,
...
unsigned int vlink_BuiltInRawPlugin_get_serialized_key_max_size_ex(PRESTypePluginEndpointData endpoint_data,
                                                                      RTIBool *overflow, RTIBool include_encapsulation,
                                                                      RTIEncapsulationId encapsulation_id,
                                                                      unsigned int current_alignment) {
  unsigned int initial_alignment = current_alignment;
  ...
  current_alignment += vlink_BuiltInRawPlugin_get_serialized_sample_max_size_ex(endpoint_data, overflow, RTI_FALSE,
                                                                                   encapsulation_id, current_alignment);
...
unsigned int vlink_BuiltInRawPlugin_get_serialized_key_max_size(PRESTypePluginEndpointData endpoint_data,
  ...
  size = vlink_BuiltInRawPlugin_get_serialized_key_max_size_ex(endpoint_data, &overflow, include_encapsulation,
                                                                  encapsulation_id, current_alignment);
...
RTIBool vlink_BuiltInRawPlugin_serialized_sample_to_key(PRESTypePluginEndpointData endpoint_data,
                                                           vlink_BuiltInRaw *sample, struct RTICdrStream *stream,
                                                           RTIBool deserialize_encapsulation, RTIBool deserialize_key,
                                                           void *endpoint_plugin_qos) {
  char *position = NULL;
...
struct PRESTypePlugin *vlink_BuiltInRawPlugin_new(void) {
  ...
  plugin->finalizeOptionalMembersFnc =
      (PRESTypePluginFinalizeOptionalMembersFunction)vlink_BuiltInRaw_finalize_optional_members;

  plugin->serializeFnc = (PRESTypePluginSerializeFunction)vlink_BuiltInRawPlugin_serialize;
  plugin->deserializeFnc = (PRESTypePluginDeserializeFunction)vlink_BuiltInRawPlugin_deserialize;
  plugin->getSerializedSampleMaxSizeFnc =
      (PRESTypePluginGetSerializedSampleMaxSizeFunction)vlink_BuiltInRawPlugin_get_serialized_sample_max_size;
  plugin->getSerializedSampleMinSizeFnc =
      (PRESTypePluginGetSerializedSampleMinSizeFunction)vlink_BuiltInRawPlugin_get_serialized_sample_min_size;
  ...
  */
  ...
  plugin->getSerializedSampleSizeFnc =
      (PRESTypePluginGetSerializedSampleSizeFunction)vlink_BuiltInRawPlugin_get_serialized_sample_size;
