...
void DdsrSubscriberImpl::init() {
  ...
  if VUNLIKELY (!participant_ || !topic_) {
    VLOG_E("DdsrSubscriberImpl::init(): participant/topic creation failed; subscriber left uninitialised.");
...
bool DdsrSubscriberImpl::is_suspend() const { return has_suspend.load(std::memory_order_acquire); }
