...
function vlink_progress_summary_available() {
    [ -n "${GITHUB_STEP_SUMMARY:-}" ] &&
        [ -d "$(dirname "$GITHUB_STEP_SUMMARY")" ] &&
        [ -w "$(dirname "$GITHUB_STEP_SUMMARY")" ]
}

function vlink_progress_escape_markdown() {
    local value="$1"
    value="${value//\\/\\\\}"
    value="${value//|/\\|}"
    printf '%s' "$value"
...
function vlink_progress_done_count() {
    local count=0
    local status
...
function vlink_progress_render() {
...
function vlink_progress_step_index() {
    local label="$1"
    local i
...
function vlink_progress_set_status() {
    local label="$1"
    local status="$2"
    local index
...
function vlink_progress_group_start() {
    local label="$1"
...
function vlink_progress_group_end() {
    if [ "$vlink_progress_group_open" -eq 1 ]; then
        if [ -n "${GITHUB_ACTIONS:-}" ]; then
            printf '::endgroup::\n'
        fi
...
function vlink_progress_complete() {
    local label="$1"
    vlink_progress_group_end
