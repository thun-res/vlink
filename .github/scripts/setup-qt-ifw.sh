...
download() {
  ...
  for attempt in 1 2 3 4 5; do
    if curl -fsSL --connect-timeout 30 --speed-limit 1024 --speed-time 60 "${url}" -o "${output}"; then
      return 0
    fi
    rm -f "${output}"
    sleep "$((attempt * 5))"
...
extract_archive() {
  ...
  download "${repo}/${file}" "${path}"
  download "${repo}/${file}.sha1" "${path}.sha1"
  echo "$(cat "${path}.sha1")  ${path}" | shasum -a 1 --status -c -
  mkdir -p "${destination}"
  (cd "${destination}" && cmake -E tar xf "${path}")
  rm -f "${path}" "${path}.sha1"
}
...
if test -x "${qt_dir}/bin/qmake" &&
  test -x "${ifw_dir}/bin/binarycreator"; then
