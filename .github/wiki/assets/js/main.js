...
(() => {
  ...
  const checkEnvItems = [
    ...
    ['VLINK_LOG_FLUSH_DELAY', '', 'Async sink flush interval in milliseconds; Quill also uses it as its minimum fsync interval.', false],
    ...
    ['VLINK_LOG_BLOCK_SYNC', '', 'When 1, producer threads block on a full queue; when 0, the backend may drop records.', false],
    ['VLINK_LOG_WRITE_DEPTH', '', 'spdlog: global queue records; Quill: bytes per producer queue (default 128 KiB, minimum 8 KiB).', false],
