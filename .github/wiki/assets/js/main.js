...
(() => {
  ...
  const checkDiagItems = [
    ...
    ['* Check VLink discover IP available...', 'ok', 'VLINK_DISCOVER_IP is empty, found 2 IP Address', 100],
  ...
  const checkEnvItems = [
    ...
    ['VLINK_LOG_OPEN_APPEND', '', 'When 1, startup continues the active/latest file; when 0, it starts a new file or rotates the active file.', false],
    ['VLINK_LOG_BLOCK_SYNC', '', 'When 1, producer threads block on a full queue; when 0, ordinary records may be dropped. Error/Fatal remain protected.', false],
    ['VLINK_LOG_WRITE_DEPTH', '', 'LoggerBackend MessageLoop dispatcher queue depth in records (default 8192).', false],
    ...
    ['VLINK_DISCOVER_IP', '', 'Local IPv4 addresses for discovery multicast (comma or space separated); empty uses every UP non-loopback interface.', false],
