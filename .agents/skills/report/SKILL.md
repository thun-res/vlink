...
name: report
description: >-
  深度分析 VLink 的项目定位、技术成熟度、发展活跃度与行业前景,结合
  报告当日的具身智能、自动驾驶、ROS 2、DDS/Zenoh/iceoryx 和边缘计算
  背景,形成带来源的中文优势/劣势/机会/风险与路线建议。用户要求"前沿
  检查报告"、"分析 VLink 前景"、"行业定位"、"活跃度分析"或"与 ROS2/
  自动驾驶/具身智能结合评估"时使用。
---
...
6. 用户要求完整报告或明确要求落盘时,将结果写入
   `.agents/cache-report/report-vlink-<YYYYMMDD>.md`;窄范围问题直接
   回答。除非用户另行要求,不改源码、文档、Issue、PR 或 roadmap。
