...
- 禁止修改 `.clang-format`、`.clang-tidy`、cpplint 过滤集或 coverage
  `EXCLUDES` 来规避告警;优先修代码。
- 静态检查确需豁免时保持最小范围,注释必须带具体检查名。
- **lib**:
  1. 能按仓库惯用写法修好的,改代码;
...
- 重构、构建与运行的授权边界以根 `AGENTS.md` 规则 3、9 为准,
  本分册不重复维护。
...
同步触发与处置遵循根 `AGENTS.md` 规则 10;完整范围只在 `/sync-review`
维护,本分册不重复列举。
