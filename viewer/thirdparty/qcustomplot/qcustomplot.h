...
enum ExportPen {
  ...
  ,
  epAllowCosmetic  ///< Cosmetic pens are exported normally (e.g. in PDF exports, cosmetic pens always appear as 1 pixel
                   ///< on screen, independent of viewer zoom level)
};
...
enum PlottingHint {
  ...
  phFastPolylines = 0x001  ///< <tt>0x001</tt> Graph/Curve lines are drawn with a faster method. This reduces the
                           ...
                           ///<                joins, thus is most effective for pen sizes larger than 1. It is only
                           ///<                used for solid line pens.
  ,
...
enum Interaction {
  ...
  ,
  iMultiSelect = 0x004  ///< <tt>0x004</tt> The user can select multiple objects by holding the modifier set by \ref
  ...
  ,
  iSelectOther = 0x080  ///< <tt>0x080</tt> All other objects are selectable (e.g. your own derived layerables, other
                        ///< layout elements,...)
  ,
  iSelectPlottablesBeyondAxisRect = 0x100  ///< <tt>0x100</tt> When performing plottable selection/hit tests, this flag
                                           ///< extends the sensitive area beyond the axis rect
};
...
enum SelectionRectMode {
  srmNone  ///< The selection rect is disabled, and all mouse events are forwarded to the underlying objects, e.g. for
           ///< axis range dragging
  ,
  srmZoom  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, the axes that are currently
           ///< set as range zoom axes (\ref QCPAxisRect::setRangeZoomAxes) will have their ranges zoomed accordingly.
  ,
  srmSelect  ///< When dragging the mouse, a selection rect becomes active. Upon releasing, plottable data points that
             ///< were within the selection rect are selected, if the plottable's selectability setting permits. (See
             ///< \ref dataselection "data selection mechanism" for details.)
  ,
  srmCustom  ///< When dragging the mouse, a selection rect becomes active. It is the programmer's responsibility to
             ///< connect according slots to the selection rect's signals (e.g. \ref QCPSelectionRect::accepted) in
             ///< order to process the user interaction.
};
...
enum SelectionType {
  ...
  ,
  stWhole  ///< Selection behaves like \ref stMultipleDataRanges, but if there are any data points selected, the entire
           ///< plottable is drawn as selected.
  ,
...
class QCP_LIB_DECL QCPPainter : public QPainter {
  ...
  */
  enum PainterMode {
    ...
    ,
    pmVectorized = 0x01  ///< <tt>0x01</tt> Mode for vectorized painting (e.g. PDF export). For example, this prevents
                         ///< some antialiasing fixes.
    ,
    pmNoCaching = 0x02  ///< <tt>0x02</tt> Mode for all sorts of exports (e.g. PNG, PDF,...). For example, this prevents
                        ///< using cached pixmap labels
    ,
    pmNonCosmetic = 0x04  ///< <tt>0x04</tt> Turns pen widths 0 to 1, i.e. disables cosmetic pens. (A cosmetic pen is
...
class QCP_LIB_DECL QCPLineEnding {
  ...
  */
  enum EndingStyle {
    ...
    ,
    esHalfBar  ///< A bar perpendicular to the line, pointing out to only one side (to which side can be changed with
               ///< \ref setInverted)
    ,
...
class QCPLabelPainterPrivate {
 ...
 protected:
  ...
  struct LabelData {
    ...
    QTransform transform;  // the transform about the label anchor which is at (0, 0). Does not contain final absolute
                           // x/y positioning on the plot/axis
...
#endif
  void setTickOrigin(double origin);  // hides base class method but calls baseclass implementation ("using" throws off
                                      // IDEs and doxygen)
...
class QCP_LIB_DECL QCPAxisTickerFixed : public QCPAxisTicker {
  ...
  */
  enum ScaleStrategy {
    ...
    ,
    ssMultiples  ///< An integer multiple of the specified tick step is allowed. The used factor follows the base class
                 ///< properties of \ref setTickStepStrategy and \ref setTickCount.
    ,
...
class QCP_LIB_DECL QCPScatterStyle {
  ...
  */
  enum ScatterShape {
    ...
    ,
    ssDot  ///< \enumimage{ssDot.png} a single pixel (use \ref ssDisc or \ref ssCircle if you want a round shape with a
           ///< certain radius)
    ,
...
void QCPDataContainer<DataType>::remove(double sortKey) {
  ...
  if (it != end() && it->sortKey() == sortKey) {
    if (it == begin())
      ++mPreallocSize;  // don't actually delete, just add it to the preallocated block (if it gets too large, squeeze
...
typename QCPDataContainer<DataType>::const_iterator QCPDataContainer<DataType>::findBegin(double sortKey,
  ...
  if (expandedRange &&
      it != constBegin())  // also covers it == constEnd case, and we know --constEnd is valid because mData isn't empty
    --it;
  return it;
}
...
QCPRange QCPDataContainer<DataType>::valueRange(bool &foundRange, QCP::SignDomain signDomain,
  ...
  if (signDomain == QCP::sdBoth)  // range may be anywhere
  {
    for (QCPDataContainer<DataType>::const_iterator it = itBegin; it != itEnd; ++it) {
      if (restrictKeyRange && (it->mainKey() < inKeyRange.lower || it->mainKey() > inKeyRange.upper)) continue;
  ...
  */
  enum PositionType {
    ...
    ptAxisRectRatio  ///< Static positioning given by a fraction of the axis rect size (see \ref setAxisRect). For
                     ///< example, if you call setCoords(0, 0), the position will be at the top left corner of the axis
                     ///< rect. setCoords(1, 1) will be at the bottom right corner, setCoords(0.5, 0) will be
                     ///< horizontally centered and vertically at the top of the axis rect, etc. You can also go beyond
                     ///< the axis rect by providing negative coordinates or coordinates larger than 1.
    ,
...
class QCP_LIB_DECL QCustomPlot : public QWidget {
  ...
  */
  enum RefreshPriority {
    rpImmediateRefresh  ///< Replots immediately and repaints the widget immediately by calling QWidget::repaint() after
                        ///< the replot
    ,
    ...
    ,
    rpQueuedReplot  ///< Queues the entire replot for the next event loop iteration. This way multiple redundant replots
                    ///< can be avoided. The actual replot is then done with \ref rpRefreshHint priority.
  };
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable, int *dataIndex) const {
  ...
  double resultDistance = mSelectionTolerance;  // only regard clicks with distances smaller than mSelectionTolerance as
...
ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const {
  ...
  foreach (QCPAbstractItem *item, mItems) {
    ...
    if (!currentItem ||
        (onlySelectable &&
         !currentItem->selectable()))  // we could have also passed onlySelectable to the selectTest function, but
                                       // checking here is faster, because we have access to QCPAbstractItem::selectable
      continue;
...
class QCP_LIB_DECL QCPColorGradient {
  ...
  */
  enum GradientPreset {
    ...
    ,
    gpIon  ///< Half hue spectrum from black over purple to blue and finally green (creates banding illusion but allows
           ///< more precise magnitude estimates)
    ,
    gpThermal  ///< Colors suitable for thermal imaging, ranging from dark blue over purple to orange, yellow and white
    ,
    gpPolar  ///< Colors suitable to emphasize polarity around the center, with blue for negative, black in the middle
             ///< and red for positive values
    ,
    gpSpectrum  ///< An approximation of the visible light spectrum (creates banding illusion but allows more precise
                ///< magnitude estimates)
    ,
    gpJet  ///< Hue variation similar to a spectrum, often used in numerical visualization (creates banding illusion but
...
class QCP_LIB_DECL QCPErrorBars : public QCPAbstractPlottable, public QCPPlottableInterface1D {
 ...
 protected:
  ...
  void getVisibleDataBounds(QCPErrorBarsDataContainer::const_iterator &begin,
                            QCPErrorBarsDataContainer::const_iterator &end, const QCPDataRange &rangeRestriction) const;
