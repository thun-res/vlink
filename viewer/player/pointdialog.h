...
class PointDialog : public QDialog {
 ...
 public:
  ...
  void reload_file(const QString& bag_path);

  void save_file();

  void set_date_time(uint64_t data_time = 0);
 ...
 private slots:
  void on_toolButton_jump_clicked();
 ...
 private:
  ...
  class QLineEdit* line_edit_{nullptr};

  QFile txt_file_;

  uint64_t date_time_{0};

 signals:
  void jump_point(int64_t timestamp);
};
