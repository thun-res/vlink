...
static constexpr auto kDdstDiscoveryTimeout = 3s;
static constexpr auto kDdsTeardownGrace = 10ms;

namespace {

struct ScopedDdstTeardownGrace {
  ~ScopedDdstTeardownGrace() { std::this_thread::sleep_for(kDdsTeardownGrace); }
};

}  // namespace

#undef TEST_CASE
#define TEST_CASE(name) TEST_CASE_FIXTURE(ScopedDdstTeardownGrace, name)
...
TEST_SUITE("ddst-pubsub") {
  TEST_CASE("bytes payload is received intact") {
    ...
    CHECK(pub.wait_for_subscribers(kDdstDiscoveryTimeout));
    CHECK(pub.has_subscribers());

    Bytes payload{0xAB, 0xCD, 0xEF};
    CHECK(pub.publish(payload));

    CHECK(common_test::wait_until([&received] { return received.load(std::memory_order_acquire); },
                                  kDdstDiscoveryTimeout));
...
TEST_SUITE("ddst-method") {
  TEST_CASE("fire and forget send increments server receive counter") {
    ...
    CHECK(client.wait_for_connected(kDdstDiscoveryTimeout));
    std::this_thread::sleep_for(20ms);
  ...
  TEST_CASE("invoke returns correct response via multiple overloads") {
    ...
    SUBCASE("sync ref overload") {
      std::string out;
      CHECK(client.invoke("pong", out, kDdstDiscoveryTimeout));
      CHECK(out == "travo:pong");
    }

    SUBCASE("async future") {
      auto fut = client.async_invoke("async");
      REQUIRE(fut.wait_for(kDdstDiscoveryTimeout) == std::future_status::ready);
  ...
  TEST_CASE("async callback invoke delivers response") {
    ...
    CHECK(common_test::wait_until([&got] { return got.load(std::memory_order_acquire); }, kDdstDiscoveryTimeout));
...
TEST_SUITE("ddst-field") {
  TEST_CASE("setter and getter exchange values via all access patterns") {
    ...
    SUBCASE("wait for value blocks until setter publishes") {
      ...
      CHECK(getter.wait_for_value(kDdstDiscoveryTimeout));
      auto v = getter.get();
      REQUIRE(v.has_value());
      CHECK((*v)[0] == 0xF0);
    ...
    SUBCASE("listen callback is invoked on value change") {
      ...
      CHECK(common_test::wait_until([&notified] { return notified.load(std::memory_order_acquire); },
                                    kDdstDiscoveryTimeout));
    }
    ...
    SUBCASE("late getter receives cached value from setter") {
      ...
      CHECK(late_getter.wait_for_value(kDdstDiscoveryTimeout));
...
TEST_SUITE("ddst-security") {
  ...
  TEST_CASE("asymmetric rsa-oaep encrypted bytes round trip via ddst") {
    ...
    try {
      ...
      if (pub.wait_for_subscribers(kDdstDiscoveryTimeout)) {
  ...
  TEST_CASE("asymmetric mismatched private key fails to decrypt over ddst") {
    ...
    try {
      ...
      if (pub.wait_for_subscribers(kDdstDiscoveryTimeout)) {
        ...
        CHECK_FALSE(common_test::wait_until([&received] { return received.load(std::memory_order_acquire); }, 2s));
      }
    } catch (const std::exception&) {
      return;
    }
