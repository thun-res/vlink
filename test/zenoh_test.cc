...
TEST_SUITE("zenoh-method") {
  ...
  TEST_CASE("client connection is reported via detect callback") {
    ...
    CHECK(common_test::wait_until([&connected_event] { return connected_event.load(std::memory_order_acquire); }, 3s));
  }
...
TEST_SUITE("zenoh-init") {
  ...
  TEST_CASE("publisher and server can coexist in same session") {
    ...
    CHECK(common_test::wait_until([&sub_got] { return sub_got.load(std::memory_order_acquire) == 42; }, 2s));
  }
  ...
  TEST_CASE("two distinct flatbuffers types coexist on separate topics") {
    ...
    try {
      ...
      CHECK(common_test::wait_until(
          [&got_msg, &got_req] {
            return got_msg.load(std::memory_order_acquire) && got_req.load(std::memory_order_acquire);
          },
          3s));
    } catch (const std::exception&) {
      return;
