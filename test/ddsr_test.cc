...
static constexpr auto kDdsrDiscoveryTimeout = 3s;
static constexpr auto kDdsTeardownGrace = 10ms;

namespace {

struct ScopedDdsrTeardownGrace {
  ~ScopedDdsrTeardownGrace() { std::this_thread::sleep_for(kDdsTeardownGrace); }
};

}  // namespace

#undef TEST_CASE
#define TEST_CASE(name) TEST_CASE_FIXTURE(ScopedDdsrTeardownGrace, name)
...
TEST_SUITE("ddsr-pubsub") {
  TEST_CASE("bytes payload is received intact") {
    ...
    CHECK(pub.wait_for_subscribers(kDdsrDiscoveryTimeout));
...
TEST_SUITE("ddsr-method") {
  TEST_CASE("fire and forget send increments server receive counter") {
    ...
    CHECK(client.wait_for_connected(kDdsrDiscoveryTimeout));
    std::this_thread::sleep_for(20ms);
  ...
  TEST_CASE("invoke returns correct response via multiple overloads") {
    ...
    SUBCASE("sync ref overload") {
      std::string out;
      CHECK(client.invoke("pong", out, kDdsrDiscoveryTimeout));
      CHECK(out == "rti:pong");
    }

    SUBCASE("async future") {
      auto fut = client.async_invoke("async");
      REQUIRE(fut.wait_for(kDdsrDiscoveryTimeout) == std::future_status::ready);
  ...
  TEST_CASE("async callback invoke delivers response") {
    ...
    CHECK(common_test::wait_until([&got] { return got.load(std::memory_order_acquire); }, kDdsrDiscoveryTimeout));
...
TEST_SUITE("ddsr-field") {
  TEST_CASE("setter and getter exchange values via all access patterns") {
    ...
    SUBCASE("late getter receives cached value from setter") {
      ...
      CHECK(late_getter.wait_for_value(kDdsrDiscoveryTimeout));
...
TEST_SUITE("ddsr-security") {
  ...
  TEST_CASE("asymmetric rsa-oaep encrypted bytes round trip via ddsr") {
    ...
    try {
      ...
      if (pub.wait_for_subscribers(kDdsrDiscoveryTimeout)) {
