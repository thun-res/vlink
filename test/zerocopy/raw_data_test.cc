...
TEST_SUITE("zerocopy-RawData") {
  ...
  TEST_CASE("deep_copy from raw pointer rejects self buffer and resizes owned storage") {
    ...
    CHECK(rd.deep_copy(larger.data(), larger.size()));
    CHECK(rd.is_owner());
    CHECK_EQ(rd.size(), larger.size());
    CHECK(region_matches(larger.data(), rd.data(), larger.size()));
  }
