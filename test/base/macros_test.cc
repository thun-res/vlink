...
class SingletonProbe final {
 public:
  explicit SingletonProbe(int /*tag*/) {}

 private:
  VLINK_SINGLETON_CHECK(SingletonProbe)
};

struct VLINK_EXPORT_AND_ALIGNED(64) AlignedProbe {
  char c{0};
};
...
TEST_SUITE("base-Macros") {
  ...
  TEST_CASE("VLINK_SINGLETON_CHECK throws on the second instantiation") {
    SingletonProbe first(1);

    CHECK_THROWS_AS(SingletonProbe(2), std::runtime_error);
  }

  TEST_CASE("VLINK_EXPORT_AND_ALIGNED applies the requested alignment") { CHECK_EQ(alignof(AlignedProbe), 64u); }
}
