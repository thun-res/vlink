...
TEST_SUITE("base-ThreadPool") {
  ...
  TEST_CASE("get_task_count is non-negative") {
    ...
    CHECK(pool.get_task_count() <= pool.get_max_task_count());
  ...
  TEST_CASE("post_task_handle returns a cancelled handle for a pre-cancelled token") {
    ThreadPool pool(1);
    ...
    auto handle = pool.post_task_handle([] {}, options);

    REQUIRE(wait_until([&handle] { return handle.is_done(); }));
    CHECK_EQ(handle.state(), TaskExecutionState::kCancelled);

    pool.shutdown();
  }

  TEST_CASE("post_task_handle returns a rejected handle after shutdown") {
    ThreadPool pool(1);
    pool.shutdown();
  ...
  TEST_CASE("get_task_count is reported for an idle lockfree pool") {
    ThreadPool pool(1, ThreadPool::kLockfreeType);

    CHECK_EQ(pool.get_task_count(), 0u);

    pool.shutdown();
  }

  TEST_CASE("post_task_handle with kReject overflow rejects a full queue") {
    SmallQueueThreadPool pool(1U);
    std::atomic<bool> started{false};
    std::atomic<bool> release{false};

    pool.post_task([&started, &release] {
    ...
    REQUIRE(wait_until([&started] { return started.load(std::memory_order_acquire); }));

    pool.post_task([] {});  // fills the single queue slot while the worker is blocked
  ...
  TEST_CASE("normal pop strategy rejects new tracked task when queued task is protected") {
    SmallQueueThreadPool pool(1U);
    pool.set_strategy(ThreadPool::kPopStrategy);
    ...
    auto protected_handle = pool.post_task_handle([] {}, protected_options);
    REQUIRE_EQ(protected_handle.state(), TaskExecutionState::kQueued);

    auto rejected = pool.post_task_handle([] {});
    CHECK_EQ(rejected.state(), TaskExecutionState::kRejected);

    release.store(true, std::memory_order_release);
    CHECK(protected_handle.wait(1000));
    CHECK_EQ(protected_handle.state(), TaskExecutionState::kCompleted);
    pool.shutdown();
  }

  TEST_CASE("normal pop strategy drops queued droppable tracked task") {
    SmallQueueThreadPool pool(1U);
    pool.set_strategy(ThreadPool::kPopStrategy);
    ...
    auto dropped = pool.post_task_handle([&executed] { executed.fetch_add(1, std::memory_order_relaxed); });
    REQUIRE_EQ(dropped.state(), TaskExecutionState::kQueued);

    auto accepted = pool.post_task_handle([&executed] { executed.fetch_add(1, std::memory_order_relaxed); });
    CHECK_EQ(dropped.state(), TaskExecutionState::kDropped);
    CHECK_EQ(accepted.state(), TaskExecutionState::kQueued);

    release.store(true, std::memory_order_release);
    CHECK(accepted.wait(1000));
    CHECK_EQ(accepted.state(), TaskExecutionState::kCompleted);
    CHECK_EQ(executed.load(std::memory_order_relaxed), 1);
    pool.shutdown();
  }

  TEST_CASE("tracked task reports failure when callback throws") {
    ThreadPool pool(1U);

    auto handle = pool.post_task_handle([] { throw std::runtime_error("tracked failure"); });

    CHECK(handle.wait(1000));
    CHECK_EQ(handle.state(), TaskExecutionState::kFailed);
    pool.shutdown();
  }

  TEST_CASE("lockfree reject overflow policy rejects a full queue") {
    TwoTaskThreadPool pool(1U, ThreadPool::kLockfreeType);
    std::atomic<bool> started{false};
    std::atomic<bool> release{false};
    ...
    REQUIRE_EQ(first.state(), TaskExecutionState::kQueued);
    REQUIRE_EQ(second.state(), TaskExecutionState::kQueued);

    PostTaskOptions reject_options;
    reject_options.overflow_policy = TaskOverflowPolicy::kReject;
    auto rejected = pool.post_task_handle([] {}, reject_options);
    CHECK_EQ(rejected.state(), TaskExecutionState::kRejected);
  ...
  TEST_CASE("lockfree pop strategy drops one queued task and accepts the newest") {
    TwoTaskThreadPool pool(1U, ThreadPool::kLockfreeType);
    pool.set_strategy(ThreadPool::kPopStrategy);
    ...
    auto first = pool.post_task_handle([&executed] { executed.fetch_add(1, std::memory_order_relaxed); });
    auto second = pool.post_task_handle([&executed] { executed.fetch_add(1, std::memory_order_relaxed); });
    REQUIRE_EQ(first.state(), TaskExecutionState::kQueued);
    REQUIRE_EQ(second.state(), TaskExecutionState::kQueued);

    auto newest = pool.post_task_handle([&executed] { executed.fetch_add(1, std::memory_order_relaxed); });
    CHECK_EQ(first.state(), TaskExecutionState::kDropped);
    CHECK_EQ(newest.state(), TaskExecutionState::kQueued);

    release.store(true, std::memory_order_release);
    CHECK(newest.wait(1000));
    CHECK_EQ(newest.state(), TaskExecutionState::kCompleted);
    CHECK_GE(executed.load(std::memory_order_relaxed), 1);
    pool.shutdown();
  }
}
