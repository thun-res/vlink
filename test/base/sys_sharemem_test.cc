...
TEST_SUITE("base-SysSharemem") {
  ...
  TEST_CASE("create twice on the same object returns false the second time") {
    const std::string name1 = make_shm_name("double_create_a");
    const std::string name2 = make_shm_name("double_create_b");
    SysSharemem shm;

    REQUIRE(shm.create(name1, 256));
    CHECK_FALSE(shm.create(name2, 256));

    shm.detach(true);
  }

  TEST_CASE("non-const data returns nullptr for a read-only mapping") {
    const std::string name = make_shm_name("ro_nonconst_data");
    SysSharemem creator;
    REQUIRE(creator.create(name, 256));

    SysSharemem reader;
    REQUIRE(reader.attach(name, SysSharemem::kReadOnly));
