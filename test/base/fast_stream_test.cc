...
TEST_SUITE("base-FastStream") {
  ...
  TEST_CASE("single-character puts overflow and grow the buffer") {
    FastStream s;
    std::string big(8200, 'Z');

    for (char c : big) {
      s.put(c);
    }

    CHECK_EQ(s.take_view().size(), 8200u);
  }

  TEST_CASE("two large writes grow through the linear increment path") {
    FastStream s;
    s << std::string(9000, 'A');
    s << std::string(9000, 'B');

    CHECK_EQ(s.size(), 18000u);
  }
}
