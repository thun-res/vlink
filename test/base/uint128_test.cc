...
TEST_SUITE("base-Uint128") {
  ...
  TEST_CASE("construct from signed negative integral sign-extends into both words") {
    Uint128 v(-1);

    CHECK_EQ(v.get_high(), UINT64_MAX);
    CHECK_EQ(v.get_low(), UINT64_MAX);
  }

#if defined(__SIZEOF_INT128__)
  TEST_CASE("native uint128 conversion preserves high and low halves") {
    const auto native = (static_cast<__uint128_t>(0x12u) << 64) | 0x34u;
    Uint128 v(native);
  ...
  TEST_CASE("public arithmetic covers carry propagation and high-word multiplication") {
    Uint128 sum(7u, UINT64_MAX);
    sum += Uint128(2u, 1u);
    CHECK_EQ(sum.get_high(), 10u);
    CHECK_EQ(sum.get_low(), 0u);

    const auto product = Uint128(UINT64_MAX, UINT64_MAX) * Uint128(0u, 2u);
    CHECK_EQ(product.get_high(), UINT64_MAX);
    CHECK_EQ(product.get_low(), UINT64_MAX - 1u);
  }
  ...
  TEST_CASE("stream output produces hexadecimal representation") {
    ...
    SUBCASE("high word pads low word to sixteen hex digits") {
      Uint128 v(0x12u, 0x34u);
      std::ostringstream oss;
      oss << v;
      CHECK_EQ(oss.str(), "0x120000000000000034");
    }

    SUBCASE("stream flags are restored after output") {
      Uint128 v(0u, 10u);
      std::ostringstream oss;
      oss << std::dec << v << ' ' << 10;
