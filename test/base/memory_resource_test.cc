...
struct PooledPair {
  ...
  PooledPair(int first, int second) : a(first), b(second) {}
};
...
static uint64_t global_pool_alloc_total() {
  uint64_t total = MemoryPool::global_instance().get_oversized_stats().alloc_count;

  for (const auto& s : MemoryPool::global_instance().get_stats()) {
    total += s.hit_count;
  }
...
static uint64_t global_pool_dealloc_total() {
  uint64_t total = MemoryPool::global_instance().get_oversized_stats().dealloc_count;

  for (const auto& s : MemoryPool::global_instance().get_stats()) {
    total += s.deallocate_count;
  }
...
TEST_SUITE("base-MemoryResource") {
  ...
  TEST_CASE("make_shared constructs the object and owns a single allocation") {
    auto sp = MemoryResource::make_shared<PooledPair>(7, 9);

    REQUIRE(sp != nullptr);
    CHECK_EQ(sp->a, 7);
    CHECK_EQ(sp->b, 9);
    CHECK_EQ(sp.use_count(), 1L);
  }

  TEST_CASE("make_shared routes its allocation through the global pool") {
    uint64_t before = global_pool_alloc_total();

    auto sp = MemoryResource::make_shared<PooledPair>(1, 2);
    REQUIRE(sp != nullptr);

    CHECK(global_pool_alloc_total() > before);
  }

  TEST_CASE("make_unique constructs the object and returns storage to the pool on destruction") {
    uint64_t alloc_before = global_pool_alloc_total();
    uint64_t dealloc_before = global_pool_dealloc_total();

    {
      auto up = MemoryResource::make_unique<PooledPair>(3, 4);

      REQUIRE(up != nullptr);
      CHECK_EQ(up->a, 3);
      CHECK_EQ(up->b, 4);
      CHECK(global_pool_alloc_total() > alloc_before);
    }

    CHECK(global_pool_dealloc_total() > dealloc_before);
  }

  TEST_CASE("make_unique returns storage to the pool when the constructor throws") {
    uint64_t alloc_before = global_pool_alloc_total();
    uint64_t dealloc_before = global_pool_dealloc_total();

    CHECK_THROWS_AS(MemoryResource::make_unique<ThrowingCtor>(), std::runtime_error);

    CHECK_EQ(global_pool_alloc_total() - alloc_before, global_pool_dealloc_total() - dealloc_before);
  }
}
