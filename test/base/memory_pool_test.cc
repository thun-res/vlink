...
TEST_SUITE("base-MemoryPool") {
  ...
  TEST_CASE("allocate rejects a non-power-of-two alignment") {
    ...
    CHECK(pool.allocate(64, 3) == nullptr);
  }

  TEST_CASE("deallocate with a non-power-of-two alignment is a safe no-op") {
    MemoryPool pool(3);

    void* p = pool.allocate(64);
    REQUIRE(p != nullptr);

    pool.deallocate(p, 64, 3);
    pool.deallocate(p, 64);
  }

  TEST_CASE("config with more than the max tier count falls back to the default pyramid") {
    std::vector<MemoryPool::Tier> tiers;

    for (size_t i = 0; i < 21; ++i) {
      tiers.push_back(MemoryPool::Tier{(i + 1) * 16, 4});
    }

    MemoryPool pool(make_config(tiers));

    CHECK(pool.get_tier_count() >= 4u);
    CHECK_NE(pool.get_tier_count(), 21u);
  }

  TEST_CASE("config with a tier smaller than the free-node size falls back") {
    MemoryPool pool(make_config({{1, 8}}));

    CHECK(pool.get_tier_count() >= 4u);
  }

  TEST_CASE("config with an overflowing tier max_size falls back") {
    MemoryPool pool(make_config({{SIZE_MAX, 8}}));
