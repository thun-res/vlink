...
TEST_SUITE("base-GraphTask") {
  ...
  TEST_CASE("invalid duplicate and missing edge mutations are ignored") {
    auto a = GraphTask::create("a", [] {});
    auto b = GraphTask::create("b", [] {});
    auto c = GraphTask::create("c", [] {});

    CHECK_THROWS(a->precede(nullptr));
    CHECK_THROWS(a->succeed(nullptr));
    CHECK_THROWS(a->remove_precede_task(nullptr));
    CHECK_THROWS(a->remove_succeed_task(nullptr));

    CHECK(a->get_succeed_task_list().empty());
    CHECK(a->get_precede_task_list().empty());

    a->precede(b);
    CHECK_THROWS(a->precede(b));
    a->succeed(c);
    CHECK_THROWS(a->succeed(c));

    CHECK_EQ(a->get_succeed_task_list().size(), 1u);
    CHECK_EQ(a->get_precede_task_list().size(), 1u);
    CHECK_EQ(b->get_precede_task_list().size(), 1u);
  ...
  TEST_CASE("status transitions pending->running->done are observed") {
    ...
    REQUIRE(common_test::wait_until([&inside] { return inside->load(); }, std::chrono::seconds(2)));
  ...
  TEST_CASE("non-std task and condition exceptions are isolated") {
    auto downstream_ran = std::make_shared<std::atomic<int>>(0);
    auto task = GraphTask::create("throw_int", [] { throw 7; });
    auto downstream = GraphTask::create("downstream", [downstream_ran] { downstream_ran->store(1); });
    task->precede(downstream);
    ...
    auto branch_ran = std::make_shared<std::atomic<int>>(0);
    auto cond = GraphTask::create_condition("cond_throw_int", []() -> int { throw 9; }, 1);
    auto branch = GraphTask::create("branch", [branch_ran] { branch_ran->store(1); });
    cond->precede(branch);

    run_graph(cond);

    CHECK_EQ(cond->get_status(), GraphTask::kStatusDone);
    CHECK_EQ(branch_ran->load(), 0);
  }
  ...
  TEST_CASE("non-std status callback exception does not prevent remaining subscribers") {
    auto throwing_calls = std::make_shared<std::atomic<int>>(0);
    auto safe_calls = std::make_shared<std::atomic<int>>(0);
    auto task = GraphTask::create("throw_int_sub", [] {});
  ...
  TEST_CASE("cancel after a completed run resets every node to inactive") {
    auto a = GraphTask::create("a", [] {});
    auto b = GraphTask::create("b", [] {});
    auto c = GraphTask::create("c", [] {});
    ...
    run_graph(a, 100);
    REQUIRE_EQ(c->get_status(), GraphTask::kStatusDone);
  ...
  TEST_CASE("clear_status_callbacks stops further notifications") {
    auto task = GraphTask::create("t", [] {});
    auto count = std::make_shared<std::atomic<int>>(0);

    task->register_status_callback([count](const std::string&, GraphTask::Status) { count->fetch_add(1); });
  ...
  TEST_CASE("kPolicyMultiple successor is invoked through the multi-policy schedule path") {
    auto ran = std::make_shared<std::atomic<int>>(0);

    auto a = GraphTask::create("a", [] {});
    auto b = GraphTask::create("b", [ran] { ran->fetch_add(1); });
    b->set_policy(GraphTask::kPolicyMultiple);
    a->precede(b);
  ...
  TEST_CASE("condition skip cascades to the unselected branch's successors") {
    auto b0_ran = std::make_shared<std::atomic<int>>(0);
    auto b0_child_ran = std::make_shared<std::atomic<int>>(0);
    auto b1_ran = std::make_shared<std::atomic<int>>(0);
    auto b1_child_ran = std::make_shared<std::atomic<int>>(0);

    auto cond = GraphTask::create_condition("cond", [] { return 0; }, 2);
    auto b0 = GraphTask::create("b0", [b0_ran] { b0_ran->fetch_add(1); });
    auto b0_child = GraphTask::create("b0_child", [b0_child_ran] { b0_child_ran->fetch_add(1); });
    auto b1 = GraphTask::create("b1", [b1_ran] { b1_ran->fetch_add(1); });
    auto b1_child = GraphTask::create("b1_child", [b1_child_ran] { b1_child_ran->fetch_add(1); });
    ...
    CHECK_EQ(b0_ran->load(), 1);
    CHECK_EQ(b0_child_ran->load(), 1);
    CHECK_EQ(b1_ran->load(), 0);
    CHECK_EQ(b1_child_ran->load(), 0);
  }

  TEST_CASE("has_cycle reports a cycle when the recursion depth limit is exceeded") {
    auto a = GraphTask::create("a", [] {});
    auto b = GraphTask::create("b", [] {});
    auto c = GraphTask::create("c", [] {});
    auto d = GraphTask::create("d", [] {});
    ...
    a->set_max_recursion_depth(1);

    auto detect_cycle = [&a] { (void)a->has_cycle(); };
    CHECK_THROWS_AS(detect_cycle(), std::runtime_error);
  }

  TEST_CASE("export_to_dot encodes condition group and policy styling") {
    auto cond = GraphTask::create_condition("selector", [] { return 0; });
    auto multiple = GraphTask::create("multi_node", [] {});
    auto waitall = GraphTask::create("wait_node", [] {});

    multiple->set_policy(GraphTask::kPolicyMultiple);
    waitall->set_policy(GraphTask::kPolicyWaitAll);
    multiple->set_group_name("groupA");
    waitall->set_group_name("groupA");

    cond->precede(multiple);
    cond->precede(waitall);

    std::string dot = cond->export_to_dot();

    CHECK_NE(dot.find("style=dashed, arrowhead=vee"), std::string::npos);
    CHECK_NE(dot.find("shape=diamond"), std::string::npos);
    CHECK_NE(dot.find("subgraph cluster_groupA"), std::string::npos);
