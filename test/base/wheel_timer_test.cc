...
TEST_SUITE("base-WheelTimer") {
  ...
  TEST_CASE("constructor rejects zero slots and zero interval") {
    CHECK_THROWS(WheelTimer(0, 5));
    CHECK_THROWS(WheelTimer(5, 0));
  }

  TEST_CASE("add rejects a zero timeout and an empty callback") {
    WheelTimer wheel(20, 10);

    CHECK_EQ(wheel.add(0, [](WheelTimer::Key) {}), -1);
    CHECK_EQ(wheel.add(20, WheelTimer::Callback{}), -1);
  }
}
