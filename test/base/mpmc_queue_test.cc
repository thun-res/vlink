...
struct ExposedMpmcQueueBase final : MpmcQueueBase {
  using MpmcQueueBase::throw_mpmc_alignment_failure;

  ExposedMpmcQueueBase() : MpmcQueueBase(1) {}
};
...
TEST_SUITE("base-MpmcQueue") {
  ...
  TEST_CASE("try_emplace constructs in place from multiple arguments") {
    MpmcQueue<Pair> q(4);
    CHECK(q.try_emplace(1, 2));
    CHECK(q.try_emplace(3, 4));
  ...
  TEST_CASE("try_emplace returns false when the queue is full") {
    MpmcQueue<Pair> q(2);
    CHECK(q.try_emplace(1, 1));
    CHECK(q.try_emplace(2, 2));
    CHECK_FALSE(q.try_emplace(3, 3));
  }

  TEST_CASE("try_emplace after notify_to_quit returns false") {
    MpmcQueue<Pair> q(4);
    q.notify_to_quit();
    CHECK_FALSE(q.try_emplace(7, 8));
    CHECK(q.empty());
  }
  ...
  TEST_CASE("wait_not_full with timeout returns false when queue stays full") {
    MpmcQueue<int> q(1);
    q.push(1);

    CHECK_FALSE(q.wait_not_full(20ms));
    CHECK(q.is_full());
  }
