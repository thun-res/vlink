...
static void busy_wait_for_cpu_active_tick() {
  auto end = std::chrono::steady_clock::now() + std::chrono::milliseconds(250);

  while (ElapsedTimer::get_cpu_active_time(ElapsedTimer::kMilli) == 0U && std::chrono::steady_clock::now() < end) {
  }
}
...
TEST_SUITE("base-ElapsedTimer") {
  ...
  TEST_CASE("get_cpu_active_time returns valid value for all accuracies") {
    busy_wait_for_cpu_active_tick();
