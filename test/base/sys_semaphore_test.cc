...
TEST_SUITE("base-SysSemaphore") {
  ...
  TEST_CASE("attach twice on the same object returns false the second time") {
    const std::string name = make_sem_name("double_attach");
    SysSemaphore sem(0);

    REQUIRE(sem.attach(name));
    CHECK_FALSE(sem.attach(name));
  }

  TEST_CASE("acquire and release before attach are safe no-ops") {
    SysSemaphore sem(0);

    CHECK_FALSE(sem.is_attached());
    CHECK_FALSE(sem.acquire(1, 100));
    sem.release(1);
  }

  TEST_CASE("blocking acquire unblocks on a cross-thread release") {
    const std::string name = make_sem_name("blocking_acquire");
    SysSemaphore sem(0);
    REQUIRE(sem.attach(name));
    ...
    CHECK(sem.acquire(1, SysSemaphore::kInfinite));
    releaser.join();
  }
}
