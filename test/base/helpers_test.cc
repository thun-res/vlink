...
TEST_SUITE("base-Helpers") {
  ...
  TEST_CASE("trim_string_view: whitespace removal without copying") {
    std::string source = "  hello  ";
    auto view = Helpers::trim_string_view(source);
    CHECK_EQ(view, "hello");
    CHECK_EQ(view.data(), source.data() + 2);
    CHECK_EQ(Helpers::trim_string_view(""), "");
    CHECK_EQ(Helpers::trim_string_view(" \t\r\n "), "");
    CHECK_EQ(Helpers::trim_string_view("\t\nhello\t\n"), "hello");
    CHECK_EQ(Helpers::trim_string_view("  a b  "), "a b");
  }

  TEST_CASE("split: splitting behavior") {
    SUBCASE("default delimiter is comma") {
    ...
    SUBCASE("single character string") {
      auto parts = Helpers::split("x", ',');
      REQUIRE_EQ(parts.size(), 1u);
      CHECK_EQ(parts[0], "x");
    }
  }

  TEST_CASE("split_view: splitting into views") {
    SUBCASE("default delimiter is comma") {
      auto parts = Helpers::split_view("a,b,c");
      REQUIRE_EQ(parts.size(), 3u);
      CHECK_EQ(parts[0], "a");
      CHECK_EQ(parts[1], "b");
      CHECK_EQ(parts[2], "c");
    ...
    SUBCASE("basic split") {
      ...
      auto parts = Helpers::split_view(src, ':');
      REQUIRE_EQ(parts.size(), 3u);
      CHECK_EQ(parts[0], "x");
      CHECK_EQ(parts[1], "y");
      CHECK_EQ(parts[2], "z");
    ...
    SUBCASE("empty string returns empty") {
      auto parts = Helpers::split_view("", ':');
      CHECK(parts.empty());
    }
    SUBCASE("consecutive delimiters skip empty") {
      auto parts = Helpers::split_view("a::b", ':');
      REQUIRE_EQ(parts.size(), 2u);
      CHECK_EQ(parts[0], "a");
      CHECK_EQ(parts[1], "b");
    }
  ...
  TEST_CASE("split_any_view: comma and space separated trimmed views") {
    SUBCASE("splits comma and space") {
      std::string src = " 192.168.1.1,10.0.0.2  172.16.0.1 ";
      auto parts = Helpers::split_any_view(src);
      REQUIRE_EQ(parts.size(), 3u);
      CHECK_EQ(parts[0], "192.168.1.1");
      CHECK_EQ(parts[1], "10.0.0.2");
      CHECK_EQ(parts[2], "172.16.0.1");
    }
    SUBCASE("non-default punctuation is not a delimiter") {
      std::string src = "192.168.1.1|10.0.0.2,172.16.0.1";
      auto parts = Helpers::split_any_view(src);
      REQUIRE_EQ(parts.size(), 2u);
      CHECK_EQ(parts[0], "192.168.1.1|10.0.0.2");
      CHECK_EQ(parts[1], "172.16.0.1");
    }
    SUBCASE("semicolon is not a default delimiter") {
      std::string src = "192.168.1.1;10.0.0.2,172.16.0.1";
      auto parts = Helpers::split_any_view(src);
      REQUIRE_EQ(parts.size(), 2u);
      CHECK_EQ(parts[0], "192.168.1.1;10.0.0.2");
    ...
    SUBCASE("blank input returns empty") {
      auto parts = Helpers::split_any_view(" \t\r\n ");
      CHECK(parts.empty());
    }
    SUBCASE("custom delimiter set") {
      auto parts = Helpers::split_any_view(" a|b/c ", "|/");
      REQUIRE_EQ(parts.size(), 3u);
      CHECK_EQ(parts[0], "a");
      CHECK_EQ(parts[1], "b");
      CHECK_EQ(parts[2], "c");
    }
    SUBCASE("empty delimiter set trims whole input") {
      auto parts = Helpers::split_any_view(" a,b c ", "");
      REQUIRE_EQ(parts.size(), 1u);
      CHECK_EQ(parts[0], "a,b c");
    }
  }

  TEST_CASE("split_any: comma and space separated strings") {
    SUBCASE("default delimiter set is comma and space") {
      auto parts = Helpers::split_any(" a, b  c ");
      REQUIRE_EQ(parts.size(), 3u);
      CHECK_EQ(parts[0], "a");
      CHECK_EQ(parts[1], "b");
      CHECK_EQ(parts[2], "c");
  ...
  TEST_CASE("path_to_string: filesystem path to string") {
    SUBCASE("temp path with component") {
      std::filesystem::path p = std::filesystem::path(vlink::Utils::get_tmp_dir()) / "test";
