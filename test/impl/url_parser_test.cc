...
TEST_SUITE("impl-UrlParser") {
  ...
  TEST_CASE("constructor rejects malformed transport prefixes") {
    CHECK_THROWS_AS(UrlParser(""), std::runtime_error);
    CHECK_THROWS_AS(UrlParser("noscheme"), std::runtime_error);
    CHECK_THROWS_AS(UrlParser(":host/path"), std::runtime_error);
    CHECK_THROWS_AS(UrlParser("ab*cd://host"), std::runtime_error);
  }

  TEST_CASE("constructor rejects malformed port values") {
    CHECK_THROWS_AS(UrlParser("someip://host:12a4/path"), std::runtime_error);
    CHECK_THROWS_AS(UrlParser("someip://host:99999/path"), std::runtime_error);
  }

  TEST_CASE("constructor rejects a duplicate query key") {
    CHECK_THROWS_AS(UrlParser("dds://host/path?k=1&k=2"), std::runtime_error);
  }

  TEST_CASE("bracketed IPv6 host is parsed and an unterminated bracket throws") {
    UrlParser p("dds://[::1]/path");
    CHECK_NE(p.get_host().find("::1"), std::string::npos);

    CHECK_THROWS_AS(UrlParser("dds://[::1/path"), std::runtime_error);
  }

  TEST_CASE("hierarchical getters throw on a non-hierarchical url") {
    UrlParser p("urn:example", kN);

    CHECK_EQ(p.get_content(), "example");
    CHECK_THROWS_AS((void)p.get_username(), std::runtime_error);
    CHECK_THROWS_AS((void)p.get_password(), std::runtime_error);
    CHECK_THROWS_AS((void)p.get_host(), std::runtime_error);
    CHECK_THROWS_AS((void)p.get_port(), std::runtime_error);
    CHECK_THROWS_AS((void)p.get_path(), std::runtime_error);
  }

  TEST_CASE("to_string emits credentials port and non-hierarchical content") {
    CHECK_NE(UrlParser("dds://u:p@host/path").to_string().find("u:p@"), std::string::npos);
    CHECK_NE(UrlParser("dds://host:7400/path").to_string().find(":7400"), std::string::npos);
    CHECK_NE(UrlParser("mailto:a@b", kN).to_string().find("mailto:"), std::string::npos);
  }

  TEST_CASE("component-map constructor rejects malformed component sets") {
    std::map<UrlParser::Component, std::string> empty_transport;
    empty_transport[UrlParser::Component::kTransport] = "";
    empty_transport[UrlParser::Component::kHost] = "host";
    empty_transport[UrlParser::Component::kPath] = "p";
    CHECK_THROWS_AS(UrlParser(empty_transport, kH, false), std::runtime_error);

    std::map<UrlParser::Component, std::string> content_on_hier;
    content_on_hier[UrlParser::Component::kTransport] = "dds";
    content_on_hier[UrlParser::Component::kContent] = "c";
    CHECK_THROWS_AS(UrlParser(content_on_hier, kH, false), std::runtime_error);

    std::map<UrlParser::Component, std::string> lone_user;
    lone_user[UrlParser::Component::kTransport] = "dds";
    lone_user[UrlParser::Component::kHost] = "host";
    lone_user[UrlParser::Component::kPath] = "p";
    lone_user[UrlParser::Component::kUsername] = "u";
    CHECK_THROWS_AS(UrlParser(lone_user, kH, false), std::runtime_error);

    std::map<UrlParser::Component, std::string> missing_path;
    missing_path[UrlParser::Component::kTransport] = "dds";
    missing_path[UrlParser::Component::kHost] = "host";
    CHECK_THROWS_AS(UrlParser(missing_path, kH, false), std::runtime_error);
  }

  TEST_CASE("component-map constructor builds a non-hierarchical url with credentials") {
    std::map<UrlParser::Component, std::string> non_hier;
    non_hier[UrlParser::Component::kTransport] = "mailto";
    non_hier[UrlParser::Component::kContent] = "user@example.com";

    UrlParser p(non_hier, kN, false);

    CHECK_EQ(p.get_transport(), "mailto");
    CHECK_EQ(p.get_content(), "user@example.com");

    std::map<UrlParser::Component, std::string> with_auth;
    with_auth[UrlParser::Component::kTransport] = "dds";
    with_auth[UrlParser::Component::kHost] = "host";
    with_auth[UrlParser::Component::kPath] = "p";
    with_auth[UrlParser::Component::kUsername] = "u";
    with_auth[UrlParser::Component::kPassword] = "pw";

    UrlParser auth(with_auth, kH, false);

    CHECK_EQ(auth.get_username(), "u");
    CHECK_EQ(auth.get_password(), "pw");
  }
}
