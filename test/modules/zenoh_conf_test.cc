...
TEST_SUITE("modules-ZenohConf") {
  ...
  TEST_CASE("invalid values qos miss and stream output cover branches") {
    ZenohConf conf("addr", "evt", 3, "missing_qos", "tcp");
    conf.shm = "true";
    conf.shm_mode = "lazy";
    conf.shm_size = "2M";
    conf.shm_threshold = "4K";
    conf.shm_loan_threshold = "1K";
    conf.shm_blocking = "false";

    std::ostringstream oss;
    oss << conf;
    CHECK(oss.str().find("ZenohConf:") != std::string::npos);
    CHECK(oss.str().find("[shm]true") != std::string::npos);
    CHECK(oss.str().find("[shm_blocking]false") != std::string::npos);

    CHECK_FALSE(ZenohConf("").is_valid());
