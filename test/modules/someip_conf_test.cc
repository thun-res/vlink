...
TEST_SUITE("modules-SomeipConf") {
  ...
  TEST_CASE("invalid configs url parsing and stream output cover branches") {
    SomeipConf::Groups groups = {0x0001, 0x0002};
    SomeipConf conf(0x1234, 0x5678, groups, 0x0099, true);

    std::ostringstream conf_stream;
    conf_stream << conf;
    CHECK(conf_stream.str().find("SomeipConf:") != std::string::npos);
    CHECK(conf_stream.str().find("0x0001") != std::string::npos);
    CHECK(conf_stream.str().find("|") != std::string::npos);
    CHECK(conf_stream.str().find("[field]1") != std::string::npos);

    CHECK_FALSE(SomeipConf(0, 1, 1).is_valid());
    CHECK_FALSE(SomeipConf(1, 0, 1).is_valid());

    SomeipConf pub_without_group(1, 1, SomeipConf::Groups{}, 0, false);
    CHECK(pub_without_group.parse(kPublisher));
    CHECK_FALSE(pub_without_group.is_valid());

    SomeipConf setter_without_field(1, 1, groups, 1, false);
    CHECK(setter_without_field.parse(kSetter));
    CHECK_FALSE(setter_without_field.is_valid());

    Url rpc("someip://4660/22136?method=1");
    CHECK(rpc.parse(kClient));

    Url event("someip://4660/22136?groups=1|2||3&event=16&field=1");
    REQUIRE(event.parse(kGetter));
    const auto* parsed = static_cast<const SomeipConf*>(event.get_target());
    REQUIRE(parsed != nullptr);
    CHECK_EQ(parsed->groups.size(), 3u);
    CHECK_EQ(parsed->event, 16);
    CHECK(parsed->field);

    CHECK_FALSE(Url("someip://4660").parse(kClient));
    CHECK_FALSE(Url("someip:///22136?method=1").parse(kClient));
  }
}
