...
TEST_SUITE("extension-DynamicData") {
  ...
  TEST_CASE("copy and move of an empty instance keep the type view empty") {
    DynamicData empty;

    DynamicData copied(empty);
    CHECK(copied.is_empty());
    CHECK(copied.get_type().empty());

    DynamicData moved(std::move(empty));
    CHECK(moved.is_empty());
    CHECK(moved.get_type().empty());

    DynamicData assigned;
    assigned.load("int", 1);
    assigned = copied;
    CHECK(assigned.is_empty());
    CHECK(assigned.get_type().empty());
  }
  ...
  TEST_CASE("copy self-assignment keeps data intact") {
    DynamicData dd;
    dd.load("int", 88);
    DynamicData& self = dd;
    DynamicData& ref = (dd = self);
    CHECK_EQ(&ref, &dd);
    CHECK_EQ(dd.as<int>(), 88);
    CHECK_EQ(dd.get_type(), "int");
  }
  ...
  TEST_CASE("move self-assignment keeps data intact") {
    DynamicData dd;
    dd.load("int", 44);
    DynamicData& self = dd;
    DynamicData& ref = (dd = std::move(self));
    CHECK_EQ(&ref, &dd);
    CHECK_EQ(dd.as<int>(), 44);
    CHECK_EQ(dd.get_type(), "int");
  }
  ...
  TEST_CASE("operator<< rejects header without null terminator") {
    std::array<uint8_t, DynamicData::get_offset()> raw{};
    raw.fill('x');
    Bytes bytes = Bytes::deep_copy(raw.data(), raw.size());

    DynamicData dd;
    const bool de_ok = (dd << bytes);
    CHECK_FALSE(de_ok);
    CHECK(dd.is_empty());
    CHECK(dd.get_type().empty());
  }
