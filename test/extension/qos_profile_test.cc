...
TEST_SUITE("extension-QosProfile") {
  TEST_CASE("every named profile has valid=true") {
    ...
    SUBCASE("kAlarm") { CHECK(QosProfile::kAlarm.valid); }
    SUBCASE("kCommand") { CHECK(QosProfile::kCommand.valid); }
    SUBCASE("kLog") { CHECK(QosProfile::kLog.valid); }
  }
  ...
  TEST_CASE("kEvent is reliable keep-last-5 volatile sync realtime") {
    ...
    CHECK_EQ(q.liveliness.kind, Qos::Liveliness::kAutomatic);
  ...
  TEST_CASE("kCommand is reliable keep-last-1 volatile sync realtime") {
    const Qos& q = QosProfile::kCommand;
    CHECK_EQ(q.reliability.kind, Qos::Reliability::kReliable);
    CHECK_EQ(q.history.kind, Qos::History::kKeepLast);
    CHECK_EQ(q.history.depth, 1);
    CHECK_EQ(q.durability.kind, Qos::Durability::kVolatile);
