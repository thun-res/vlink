...
class InvalidStatus final : public Status::Base {
 public:
  [[nodiscard]] Status::Type get_type() const override { return static_cast<Status::Type>(255); }

  [[nodiscard]] std::string get_string() const override { return "InvalidStatus"; }
};
...
TEST_SUITE("extension-Status") {
  ...
  TEST_CASE("Base ostream dispatches every status type") {
    const std::vector<Status::BasePtr> statuses{
        std::make_shared<Status::Unknown>(),
        std::make_shared<Status::PublicationMatched>(),
        std::make_shared<Status::OfferedDeadlineMissed>(),
        std::make_shared<Status::OfferedIncompatibleQos>(),
        std::make_shared<Status::LivelinessLost>(),
        std::make_shared<Status::SubscriptionMatched>(),
        std::make_shared<Status::RequestedDeadlineMissed>(),
        std::make_shared<Status::LivelinessChanged>(),
        std::make_shared<Status::SampleRejected>(),
        std::make_shared<Status::RequestedIncompatibleQos>(),
        std::make_shared<Status::SampleLost>(),
    };
  ...
  TEST_CASE("Base ostream handles invalid and null statuses") {
    InvalidStatus invalid;
    std::ostringstream invalid_oss;
    invalid_oss << invalid;
    CHECK_EQ(invalid_oss.str(), "Unknown");

    Status::BasePtr null_status;
    std::ostringstream null_oss;
    null_oss << null_status;
    CHECK_EQ(null_oss.str(), "null");
  }
}
